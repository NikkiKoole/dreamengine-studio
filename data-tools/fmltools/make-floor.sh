...
#
#   data-tools/fmltools/make-floor.sh <file.fml> <cartname> [floor] [scale] [maxfurn]
#
...
#   SATURATE=2.2 POSTERIZE=5 OUTLINE=1 MAX=24 data-tools/fmltools/make-floor.sh ...
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"   # data-tools/fmltools/ → repo root
cd "$ROOT"
...
node data-tools/fmltools/fml2cart.js "$FML" --out "$CART" --scale "$SCALE" --maxfurn "$MAXFURN" --floor "$FLOOR"

echo "▸ bake art  (max ${MAX}px, saturate ${SATURATE}, posterize ${POSTERIZE}, outline ${OUTLINE})"
node data-tools/fmltools/fml-assets.js "$FML" --max "$MAX" --saturate "$SATURATE" --posterize "$POSTERIZE" $OUTFLAG --out "$ASSETS"

echo "▸ embed sprites"
node data-tools/fmltools/fml-sprites.js --manifest "$ASSETS/manifest.json" --out "$CART"
...
if(!idx.some(e=>e.file===file)){
  idx.push({title:name, description:"A Floorplanner .fml walked as a top-down level (built by data-tools/fmltools/make-floor.sh). Walls with walkable doorways and real door/window sprites, each room a distinct floor colour, furniture as baked top-down pixel sprites. WASD/arrows move, T toggles sprites vs boxes.", file, kind:["tech-demo","toy"], teaches:[]});
