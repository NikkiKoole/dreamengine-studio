...
(15) **the contemporary ReBirth — a rack that clones TECHNIQUES, not machines** (`design/contemporary-rebirth.md`; rungs A + B shipped: `multiband()` + `sample_shift()`/`harmonize_mic()`, showcased by `hyperbox` + `voxshift`; NEXT = Rung C, the beat-synced buffer re-reader),
...
> **▶ ACTIVE THREAD (2026-07-26) — the CONTEMPORARY ReBirth: rungs A + B shipped, C is next.**
...
> always-on squash). The doc audited eight boxes across two candidate racks and found **six need zero
> engine work**; the three gaps became rungs. **Shipped this session:** Rung A `multiband()` /
> `instrument_multiband()` / `FX_MULTIBAND` (three-band dynamics with an UPWARD half, the OTT box, `mix`
> 0 = byte-identical bypass) with `hyperbox` as its showcase rack, and half of Rung B, `sample_shift()`
> + `harmonize_mic()` (length-preserving transpose, one PSOLA core wearing both the snap and shift
> faces, bit-identity re-proven by SHA) with `voxshift` as the acceptance probe. **The other half of
> Rung B failed honestly and is parked WITH NUMBERS** (an octave in the singer's own voice: formants
> hold, f0 wobbles 170-300 Hz vs 5 Hz raw, because a grain carrying source periodicity still sounds at
> the source pitch) — that is a spike, not a flag, and `hyperbox` v2's real voice box waits on it.
> **NEXT = Rung C: `beatfx(mode, bars, mix)`**, a beat-synced buffer re-reader (halftime / beat repeat)
> and the biggest of the three; it must be **ride-safe** (parameters read per sample, never re-allocated
> per call) or it lands on the wrong side of `lint-fx-frame.js`. Resume at
...
> captured live, not replayed from a `.rec`), and the hip-hop rack, whose hard part is not DSP but
> **where the loop audio legally comes from**. Hot files if you take this: `runtime/sound.h` (targeted
> `Edit`s only, never a full `Write`). Adjacent records: [`design/rebirth-classic.md`](design/rebirth-classic.md)
> (the RB-338 pilot whose chassis both racks reuse), [`design/transparent-autotune.md`](design/transparent-autotune.md)
