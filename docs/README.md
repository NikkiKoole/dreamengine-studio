...
│   ├── action-plan.md   LIVING ACTION LIST (2026-06): the do-able asks from the field-notes journal + the Tinyjam brainstorms, prioritized & checkable. Tier 0 (save/share lz-string codec = the tinydaws seed-code; build-field-notes; rebake thumbs), Tier 1 knowledge infra (per-cart-metadata→generated index.json = THE keystone that kills the index.json multi-agent conflict; cart lifecycle/curated views; build-context; frontmatter; stale-doc/promote-candidate), Tier 2 GATED audio product (JSON-diff/RLHF export, bias knobs, visualizer, native iOS/AUv3), maker decisions (Tinyjam vs tinydaws naming), and the systemic cart-polish backlog (shared touch-joystick/pixel-button/drag-capture, adventure z-order, end-state, text overflow) distilled from field-notes/0000000-cgecking-all-carts.md
