...
│   ├── touch-notes.md  multitouch: the touch API + identity model, the release API (tapr/touch_ended_*, shipped with the touch piano), the never-wrap-rgestures verdict + gestures.h, the touchlab on-device checklist, and §7: the web phantom-touch root cause (emscripten wontfix × raylib single-removal) + fix plan
