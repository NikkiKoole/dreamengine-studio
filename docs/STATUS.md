...
_Last updated: 2026-07-30 (**THE CONTEMPORARY REBIRTH, RUNGS A + B — a post-hardware rack clones TECHNIQUES, not machines. The design record ([contemporary-rebirth.md](design/contemporary-rebirth.md)) audited eight boxes across two candidate racks and found **six need zero engine work**; the three gaps became rungs, and two landed the same day. **RUNG A: `multiband(low, mid, high, up, mix)`** + `instrument_multiband(slot, …)`, insert kind `FX_MULTIBAND` (18): three-band dynamics with an **UPWARD** half, which is the "OTT" sound of a modern master (the quiet detail gets pushed up, so the mix is permanently "on"). `mix` 0 = bypass, **byte-identical**, because the bands sum back to the input. Named `multiband`, not `squash`: `build-all` caught five carts already declaring a local `squash` (squash-and-stretch is animation vocabulary, the `map` trap's second instance). **RUNG B: `sample_shift(slot, semitones)`** (offline, ±24) + **`harmonize_mic(semitones, voices)`** (live, a 1-3 voice stack), both transposing while **keeping the take's length**, which playing a slot at a higher note cannot do. One PSOLA core now wears both faces (snap and shift), and generalizing it was proven **bit-identical** for the shipped autotune by re-render + matching SHA, not by argument. Two showcase carts: **`hyperbox`**, the tiny hyperpop rack (pitch-snapped voice, seven-saw unison wall, ratcheted 909 kick, a master chain with **no bypass drawn**), and **`voxshift`**, the acceptance probe (one captured voice four ways: raw, snapped, +12 formants HELD, +12 formants FOLLOWING; f0 within 0.5 Hz of target at +3/+5/+7/+12/-12). **Half of Rung B failed honestly and is parked with numbers**: "an octave up in the singer's OWN voice" is not a flag on this code. Re-spacing epochs holds the formants (F2 991 to 947) but leaves f0 wobbling 170-300 Hz against the raw take's 5 Hz, because a grain carrying the source periodicity still sounds at the source pitch however you re-space it. A correction moves the period a few percent (inaudible); an interval halves or doubles it. So the transparent SHIFT needs its own spike, exactly as the transparent CORRECTION got two. **What building the rack taught us: with a squashed master you mix for the GAPS, not the levels** (the first render measured rms -1.5 dBFS at a 1.5 dB crest, a wall with no groove left, because OTT lifts the synth's sustain into every hole the kick needed; the fix was cart-side, a decaying stab instead of a pad plus parts sitting much lower than feels right dry, landing at rms -5.3 / crest 5.2 where real hyperpop masters sit). Open: Rung C (`beatfx`, a beat-synced buffer re-reader for halftime/beat-repeat, the biggest of the three), `hyperbox` v2's real voice box, a home cart for `harmonize_mic`, and the hip-hop rack. Ledger: [audio-notes.md](design/audio-notes.md) §17 #34 + #35; recipes in [effects-recipes.md](guides/effects-recipes.md).**) Previous headline: (**THE HARMONY BRAIN GRADUATES — `runtime/harmony.h`: one 13-function roman-numeral vocab + per-style Markov transition tables (`HB_BOSSA`/`HB_COCKTAIL`) extracted from bossa/cocktail with **byte-identical pinned-seed WAV proof**, and now BIDIRECTIONAL: `hb_pick` (generate — the radios keep composing on it) + `hb_suggest` (the table read forward: ranked next-chord options + a one-word reason) + `hb_analyze` (the inverse: chords in a KNOWN key → roman numerals; out-of-vocab = honest `-1`, per the research — full auto RN analysis is unsolved). The demand-82 toy (r/musictheory's "progression analyzer/suggester", the loudest drip gap on record) shipped as **`chordwise`**: analyzer strip re-named every frame, NEXT panel with weight pips + reasons, the 13-chord palette spelled in the key, doo-wop cold-open, a 24-assert `spec()` incl. the all-12-keys round-trip (analysis inverts generation exactly). Banked en route: **spec.js was broken repo-wide** since the mic lane (missing AudioToolbox at link) — fixed, streetlab 104/0 again. Design: [harmony-brain.md](design/harmony-brain.md).**) Previous headline: (**AUDIO INPUT + THE VOCODER — the engine can HEAR and SPEAK. A device-free MIC SEAM on ALL FOUR platforms (desktop CoreAudio · web getUserMedia · iOS AVAudioSession · Android AAudio) behind one `platform.h` contract: `mic_level()` (RMS) + `mic_pitch()` (a YIN detector — octave-safe) + `mic_record()` (capture-then-freeze PCM). Then a real master-stage 12-band VOCODER — carrier×modulator cross-synthesis — with a lock-free audio-thread mic ring (`sound_extin_*`, which also opens the whole live-throughput/PEDAL tier): `vocoder()` / `vocoder_send()` / `vocoder_mic()`. New carts: `mictest` (VU+pitch), `pitchscope` (the YIN diagnostic), `humtheremin` (play by humming), `voxbox` (the REAL vocoder — sing and a saw chord speaks your words), `vocode` (deterministic synth-modulator vocoder), plus `breakchop` gained mic BEATBOX auto-chop. Determinism carve-out: [ADR-0032](decisions/0032-live-mic-effects-are-live-only.md) (live-mic-through = live-only). Design: [mic-and-sampling.md](design/mic-and-sampling.md) + [vocoder.md](design/vocoder.md). All gates green (soundcheck/tune/dc/build-all/wasm-parity); desktop+web live-verified by the maker.**) Previous headline: (**LOCKSTEP NETPLAY, RUNG 5A STEPS 1–3 — BROWSER NETPLAY LIVE-VERIFIED (two real browsers, a relay match on the home wifi; peer-left banner on the survivor's screen): two tabs (or an iPad + a laptop) meet in a room via `tools/net-relay.js` (zero-dep WS relay, blind byte-forwarding, `--serve` = cart + relay from one wifi box printing the exact two-device URL; lobby v0 = `?room=X` in the URL) over an `EM_JS` WebSocket arm in net.h with an async pre-init handshake (ROLE→HELLO→WELCOME{seed}); the live two-tab pick is the one remaining test. `node tools/net-check.js` = the one-command gate (echo mirror + netdemo pair + relay wire-protocol, all PASS). Underneath it, STEP 1: `net.h` split into `DE_NET_CORE` (rings/packets/barrier, no sockets — native AND web) + `DE_NET_BUILD` (UDP + flags + lobby, native-only as before), all I/O through the new `net_transport_send/pump` seam where step 2's WebSocket arm lands; non-blocking `net_frame_try_sync()` barrier (web stalls the tick — a browser can't block; native wraps it blocking, netdemo-identical); det clock unified for web; and an ECHO loopback fake-peer transport proves remote-input injection on both targets (`--net-echo` / `-DDE_NET_ECHO_DEFAULT`). Gates: netdemo LOCKSTEP OK, echo p2y==p1y ×300, build-all 466/466, emcc green. LAN rungs 1–3 untouched — web is a second transport NEXT to UDP. Full ledger entry below.**) Previous headline: (**LOCKSTEP NETPLAY — RUNGS 1 & 2 SHIPPED: two native builds play the SAME cart over UDP (localhost or LAN by IP), input-lockstep, carts stay 100% network-unaware — `btn(player,…)` now means "which machine": all local input (either keymap + touch) is MY player, the peer's byte comes off the wire. `runtime/net.h` (~250 lines, runtime-flag-gated like the harness: `--net-host` / `--net-join <ip>` / `--net-port`), host seeds the joiner in the handshake, NET_DELAY=3 frames input latency, GGPO-style redundant packets, BYE/timeout exits. Demo + deterministic gate: `play.js <cart> netdemo` spawns a host+joiner pair side by side and diffs their per-frame traces (LOCKSTEP OK / DESYNC, checks-and-oracles has the one-liner; netcheck clips parked at tools/clips/pong/). RUNG 2 (LAN by IP, same day): a **🌐 multiplayer button** in the editor drives host / join-by-IP (shell.js popover; main.cjs adds the `--net-*` flags), and the host resolves + shows its LAN IPv4 (`net_local_ipv4()` via `getifaddrs()`) so the joiner knows what to type — the "click host → get an address" UX, no NAT, no servers. Rung ladder + what's next: design/multiplayer-research.md.**) Same day: **TINYJAM RACK #2 SHIPPED — `yachtrack` "session desk": the yacht radio opened up — the first CHORD-FIRST rack (the chord chart is the star: mu-vocab chord row + MU-IFY/SUS-MELT, form row + gear change, drum skeleton lanes w/ session/Purdie/CR-78 drummer chairs, the 32-step sax hook cell — while bass runs / comp anticipation / ghosts / swing stay session PLAYERS behind feel knobs: the chart+interpreters model) and the first RADIO→RACK SEED HANDOFF, proven — the generator is yacht's `new_song` VERBATIM, gated by a 44-pair golden songsum corpus in `spec()` (48 asserts). Design record: design/yacht-rack.md, design→shipped in one day.** Previous headline: **THE FIRST TINYJAM RACK SHIPPED — `acidrack`: the ReBirth RB-338 (2×303 + full 909 + curated 808) as one cart with per-device FX, banks+song chain, and a SEEDED GENERATOR (8-hex song code → a whole arranged acid track, WAV-exportable). Rode in with engine work: `FILTER_DIODE`, a real TB-303 diode-ladder lowpass (~18dB/oct, bass-draining resonance, in-loop saturation — audio-notes §25) + `tools/filter-spec.js`, the filter-response oracle; tb303.c upgraded to it. Design record: design/rebirth-classic.md, design→shipped in one day.**_
