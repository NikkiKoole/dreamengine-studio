...
- **`tools/sprite-draw.js`** — shared programmatic sprite-authoring library for `.cart.js` files. Exports a 2D pixel-canvas API aligned with the C drawing API names: `blank`, `pixel`, `rectfill`, `line`, `circlefill`, `trifill`, `outlined`, `mirror`, `stamp`, `flat`, `split`, `OUT`. All 14 programmatic `.cart.js` files now `require('../sprite-draw.js')` instead of copy-pasting helpers. Three `.cart.js` authoring styles documented: (1) settings-only, (2) ASCII art with `DEFAULT_CHAR_MAP` (palette 0–15), (3) programmatic arrays via sprite-draw (palette 0–31, geometry). See `tools/sprite-draw.js` and `CLAUDE.md` → project structure.
...
**`sprite-draw.js` next steps (pick one):**
- **Gap audit** *(recommended first)* — compare `studio.h` drawing primitives against `sprite-draw.js` exports and identify which are useful for offline sprite authoring. Obvious candidates: `ovalfill` (ellipse, not just circle), `ngonfill` (hex/diamond tiles, gem shapes), `vgradient`/`hgradient` (fake shading/depth). Many C primitives (`pal`, `fillp`, `camera`, `map`, sound) are runtime-only and have no offline equivalent — "full alignment" is the wrong goal; "authoring-useful subset" is right.
- **Extend with JS-only capabilities** — things that don't exist in C land but help sprite authoring: procedural noise fill (deterministic per-pixel hash pattern, currently copy-pasted in many carts), `hflip`/`vflip`, `rotate90`, a `noise(x, y, scale)` helper. These are authoring conveniences, not runtime API.
- **Stress-test cart** — a cart that exercises the full sprite-draw API visually (all primitives, outlined, mirrored, split) to catch visual regressions and serve as a reference. Doubles as a thumbnail showcase for what the library can produce.
