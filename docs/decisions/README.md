...
| [0015](0015-effects-are-recipes-not-primitives.md) | Effects are recipes, not primitives: the GATE, not a closed roster | accepted (count superseded) |
...
| [0035](0035-free-with-one-pro-unlock.md) | Apps ship FREE with one non-consumable "Pro" unlock (WAV export + MIDI in/out + AUv3) at $4.99 (amended 2026-08-19; $9.99 was the first assumption); the wall is "it leaves the app", so the free instrument is complete (background audio, Link, saving) and nothing is degraded. No subscription, no feature ladder. Switch the two $1.99 live listings NOW, while sales are zero, because that is what deletes the receipt-based grandfathering code; a price change needs no build and no review (`asc-push.js --price`, 0 = Free), so never pull-and-re-release | accepted |
