...
- [x] **Touch-input policy — decided and shipped (2026-07-01).** Answer: opt-in **per cart, in
  code** (`touch_layout(mode, n_buttons)`), never a global setting — see the cart-polish item below
  and [`touch-controls.md`](touch-controls.md) for the full design + what's shipped.
...
- [x] **A nicer shared end-state.** The "fade to black / fade to half" at win/lose recurs and reads as
  unfinished — define one good shared end-screen treatment and apply it. **DONE (2026-07-10):
  `runtime/endcard.h`** — an eased ordered-dither curtain (in-canvas, deliberately NOT `fade()`:
  that window-space pass dims the card + text along with the world and never shows in dumps/gifs/
  baked thumbnails) + a drop-shadowed double-bordered card that pops in (`ease_back`) + a `.settled`
  flag the cart gates its own words on. **Converted (23):** larry, monkey, zak, civ, jumpstar,
  advancewars, battleship, bejeweled, boggle, boulderdash, doom, finalfight, heroes (battle-over +
  campaign-over), hotline (its `panel()` also serves the title card), incmachine, marble, monstermix,
  outrun, qbert, rivercity, sandburrow, solitaire, turfwar — the whole flat-`fade(0.5ish)`-end-screen
  sweep (`grep -ln "fade(0\.5\|fade(clamp" tools/carts/*.c`). Deliberately NOT converted: wintergames
  (fade = its event-shell scene *transition*, by design), rivercity's intro fade, monkey's mid-duel
  falter flash — transient tints, not end curtains. New game carts: use `endcard.h`, never hand-roll
  `fade(0.5)+box` (see [cart-authoring.md](../guides/cart-authoring.md) → library headers).
