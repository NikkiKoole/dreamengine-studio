...
|---|---|---|
| `title` | yes | |
| `slug` | **no — authored in `de:meta`, required by `lint-carts.js`, and deliberately not exported to `index.json`** (nothing reads it from there; the derived `file` carries the link) | **the canonical `<name>`** — the anchor from a `.cart.png` back to `tools/carts/<name>.c` (and `<name>.cart.png`). Authored, must equal the `.c` filename stem (`lint-carts.js` enforces — missing or mismatched fails); duplicates the filename *on purpose* so the link survives when the `.c` is extracted into a PNG that no longer carries its filename. The derived `file` below is `<slug>.cart.png`. Solves "the PNG doesn't know its own source" — [`editor-cart-workflow.md`](editor-cart-workflow.md) Gap 1b; unblocks the Gap-1 save-back-to-source round-trip. **Backfilled across all carts + made required 2026-07-10** (`tools/backfill-slug.js`); new carts add it as the de:meta's first field. |
| `status` | not yet | `active`(default) / `showcase` / `retired` / `archive` / `hidden` / `wip` — seeds the real featuring system (replaces the dead array-order "featured" sort). All carts migrate to `active`. `wip` = a functional but incomplete slice (e.g. a phase-1 build of a larger design); registered but not gallery-ready — flip to `active` when finished. |
