...
| **Drive a *generated city* with real streets + buildings** | `sloop` + `citygen.h` (rung 5.5c + 7 ✅ 2026-07-10) | ✅ works today (press M: a city grown by the worldgen rungs-2→5 grammar — density field → tensor arterials → district minor-street fill, SNDi-calibrated to real Rotterdam — drivable via the same `road_at()` seam, grip flips on/off street, and its minor streets are **lined with ~1300 solid procedural buildings** you crash through via the same `OB_HOUSE` path as OSM Delft. *One bounded city so far; unifying it with the N-spine's infinite world is the open reconciliation*) |
