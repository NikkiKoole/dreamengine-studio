...
## Implementation status (as of 2026-06-23) — ▼ SUPERSEDED, kept for the trail

> ⚠ **Read the ✅ table above this section instead.** Two rows below are now wrong: **row 0**
> (`de:meta.slug`) shipped 2026-07-10 and is backfilled across every cart and enforced by
> `lint-carts.js`, and **row 7** (the sprite patch/overlay layer) shipped 2026-07-10 as
> `tools/lib/sprite-patch.js` + `<name>.sprites.patch.json` + the `de:spritepatch` chunk.
> The rest of the table was still accurate when last checked.
...
| # | Fix | Status | Evidence |
|---|---|---|---|
| 0 | `de:meta.slug` field | ✅ **SHIPPED 2026-07-10** (backfilled + `lint-carts.js`-enforced) | Nothing records the source `.c`: `de:meta` has no `slug` (see `cart-metadata.md` schema); the `.png`→`.c` link is the basename convention (`main.cjs` ~843, `cart-info.js` ~59) and the `index.json` title↔file back-map (`cart-info.js` ~157). Publish is the only source write-back (`main.cjs` ~2110), and it reconstructs `<name>` from the basename. |
...
| 7 | Sprite patch/overlay layer (option D) | ✅ **SHIPPED 2026-07-10** (`tools/lib/sprite-patch.js`, per-slot overlay, `de:spritepatch`) | No diff-on-edit, no `patches` block / `.sprites.patch.json`, no fingerprint/staleness check, no promote path. `COMIC_PATCHES` (gen-rom-font.js) is the only patch-survives-rebake precedent. |
