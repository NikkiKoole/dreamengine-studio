...
| **the cart reads notes** | ✅ **since 2026-08-15** — `acidcandy.c` drains `midi_get()` every frame (was: "calls `midi_get()` / `midi_held()` zero times", which is the gap this doc was written to close). `midi_held()` is still unused, deliberately: the drain gives the rack what it needs |
