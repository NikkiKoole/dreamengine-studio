...
| **AM / ring mod** | trivial (≈10 lines native) | free | modulator ratio | AM ↔ ring depth | modulator detune / wave | metallic, robotic, clangorous bells. **Mostly covered 2026-06-14** by the `ringmod()`/`instrument_ringmod()` bus effect (`FX_RINGMOD` — fixed-Hz sine carrier); what an *engine* row would still add is the note-TRACKING modulator (ratio follows the played pitch, so the clang stays harmonic per note) |
...
>
> **Update 2026-07-02 — those doors have since opened.** Formant SHIPPED as `INSTR_VOICE`
> (2026-06-10, + the `formant()` vowel filter); reed as `INSTR_REED` (2026-06-08); membrane as
