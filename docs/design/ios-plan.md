...
| 8.5 | **a MULTI-CART app (the standalone Tinyjam) renders on iOS** | the umbrella app on a phone | sim | ✅ **done (2026-07-03)** — `build-app.js --ios` stages the dispatcher shim + per-cart wrappers into `ios/gen/app`; `project.yml` sources that directory; `ios/build.sh`/`device.sh` `APP=<manifest>`. Tinyjam's launcher (acid rack / session desk from de:meta, `>` cursor, footer) renders on the iPhone 15 sim. share-panel.md "Spike A" / next-spike #5. Open: touch back-to-launcher, on-device deploy. |
