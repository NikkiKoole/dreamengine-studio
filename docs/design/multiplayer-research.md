...
| **4. Web lobby / room UX** | **v0 shipped with step 2: the URL is the lobby** — both players open the same link, `?room=<code>` picks the room, `?relay=` overrides the default (the page's own host), and the pre-init screen shows the connect/wait status. Still open for a real v1: an in-cart room-code entry (the native lobby's Host/Join/Solo shape), a generated shareable code, and the `de_players() >= 2` gate deciding whether to net at all. | ~1–2 days | low; mostly the lobby draw, already exists for native. |
