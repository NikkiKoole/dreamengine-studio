...
| `rvbwidth` | the reverb tank is **mono** — does a wide tail need a **stereo tank** (new DSP), or can cart-land get there? | **cart-land gets there — no second tank needed.** Two plucks panned hard apart, read with `stereo-check.js`: a mono send **halves** the stereo width (0.9999 → 0.5569), but `reverb_bus_fx(tank, FX_CHORUS, …)` after the reverb recovers **0.9710** of it, because chorus writes antiphase taps to L and R. A second tank was priced at +103 KB and +0.010% of one core and **rejected as unnecessary**, which also vindicates [`stereo.md`](stereo.md)'s original call that width is "NOT new API, it's a recipe". ⚠ Nuance: `reverb_plate()` + `reverb_plate_width()` *did* land separately (2026-08-19) as the **two-pickup** variant — extra taps on the existing comb lines, 4.3 KB/tank — giving **static** width (0.74) plus real plate character on the plain master send. So the probe's verdict holds for the expensive implementation, and the cheap one earned its place on a different axis than width. [`analog-outboard-chain.md` §6](analog-outboard-chain.md) |

## ✅ Resolved: engine API was needed — the probe became the demo

| Cart | What it proved | API that shipped |
|---|---|---|
| `blendlab` | 32-color translucency works as a lookup table, **but cart-land can't read dst correctly** — its deliberately-broken `P` mode shows the last-frame `pget` feedback bloom; dst must come from the in-progress frame | **verdict in, API SHIPPED 2026-07-10** — [ADR-0031](../decisions/0031-blend-tables-before-palette.md) built `blend()` / `blend_reset()` on both renderers, from the *live* `palette[]` rather than baked, which is what let it land ahead of the palette question ([`blend-tables.md`](blend-tables.md)). The probe proved the need and the build followed. |
