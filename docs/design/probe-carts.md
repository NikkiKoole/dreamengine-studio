...
| `palettelab` | which default palette replaces the lifted PICO-8 one — and at what size? ([`palette-and-color.md`](palette-and-color.md) Layer 1/1b) | contact sheet live: shipped-32 / ENDESGA 32 / full Resurrect 64 / E32+derived, role-mapped, across swatch/ramp/sunset/portrait scenes **plus blendlab's blend scenes** (night glow, glass+fog, table grid) rebuilt from the live candidate — palette choice and blend fidelity judged together. Built on the **experimental** `palette_hex()` + 64-slot palette (see below) |
