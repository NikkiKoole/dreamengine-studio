...
| `touchlab` | does the touch stack hold up on real hardware — and do we then need a release API (`tapr`/`touch_ended_*`) and/or gesture wrappers (raylib rgestures vs cart-land)? ([`touch-notes.md`](touch-notes.md) §3–5) | 7-point on-device test card built; **blocked on hardware** (no touchscreen available yet). Run it on an iPad via Build-for-web + LAN url, write verdicts into touch-notes §5. The release-API design is parked in §3 with the touch piano as its designated second customer |
