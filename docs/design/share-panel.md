...
> **The example above is illustrative; the real schema has grown** (see the live
> `apps/tinyjam/app.json`). Fields in use as of 2026-07-03: `name`, `bundleId`, `version`,
> `carts[]`, `launcher`, **`iap.products[]`** (`{id, price, name, desc, unlocks[]}` — StoreKit),
> and **`listing`** (`{title, subtitle, keywords}` — the store metadata's machine-readable
> home, read by the editor Apps view + `aso-lint`/`aso-compose`;
> [`store-agents.md`](store-agents.md) §ASO, [`../marketing/tinyjam/app-store-listing.md`](../marketing/tinyjam/app-store-listing.md)).
> Screenshots live *beside* the manifest in `apps/<name>/screenshots/` (generated by the
> Apps view's 📸 action, consumed by `press-kit`).
...
3. **Differing screen dims** — `SCREEN_W/H` are compile-time `#define`s (`studio.h`),
   ...
   - **C — fully runtime-variable screen dims (large, the eventual home):** `SCREEN_W`
     ...
     **[responsive-layout](responsive-layout.md)** model where a cart *responds* to its viewport (pairs with the
...
`aso-lint`, `store-shots`, `store-contact`, [`press-kit`](press-kit.md)) + `build-app`/`mac-app.sh` +
