# Tinyjam Racks — follow-up brainstorm: bias knobs, the agentic RLHF loop, the visualizer
...
name "Tiny Jam"; mapping to this codebase is ours. Read [`tinyjam-racks.md`](tinyjam-racks.md) **first** —
...
The outside agent reinvented, from scratch, the **core move tinyjam-racks.md already names**: a radio
...
section + density (`tinyjam-racks.md` §"half the groove is NOT data"). A bias knob is just those gates
...
This closes the loop the original tinyjam racks motivation opened ("these boxes would let me improve
...
in `tinyjam-racks.md` §Export. **The deeper bet** — app pushes diff → agent rewrites the generator,
...
## Net effect on the tinyjam racks plan

Nothing in the build order (`tinyjam-racks.md` §Build order) changes. Three small additions slot in
...
in `tinyjam-racks.md`; this doc keeps only the deltas.
