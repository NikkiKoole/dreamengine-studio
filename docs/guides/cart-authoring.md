...
| `physics.h` | a tiny shared **verlet (position-based) toolkit**: a `PhysPt{x,y,px,py,w,r}` the cart owns (inverse-mass `w`, radius `r`; `w=0` = pinned/grabbed) + the verbs `phys_integrate` / `phys_link` (rigid distance stick) / `phys_spring` / `phys_collide` (circle↔circle) / `phys_collide_seg` (point↔segment) / `phys_bounds` (AABB clamp + bounce + floor friction) / `phys_aim` (angular spring) + `phys_grab`/`phys_kick`/`phys_vx`. The cart keeps its own arrays + step loop. Include **after** `studio.h` | any cart with **ropes, cloth, ragdolls, blobs, chains, bouncy balls, or stick-built pseudo-solids** — the recurring hand-rolled verlet loop, now shared (the math had been copy-pasted across ~10 carts). NOT a managed world or rigid-body engine — that's the parked **Layer-1 seam** in [physics-notes.md](../design/physics-notes.md); the cart owns its step + any bespoke collision/bounds | `physics` (teaches it inline), `verlet` (toolkit demo), `ragdoll`, `linerider`, `tombola` |
| `drumkit.h` | a shared **playable drum kit** — the engine has no `INSTR_KIT`, so every drum cart hand-builds ~6-16 voices from primitives. This owns the *skeleton*: a role vocab (`DK_KICK`/`DK_SNARE`/`DK_HHC`/`DK_HHO`/`DK_CLAP`/`DK_TOM_LO`/`DK_TOM_HI`/`DK_CRASH`), a fixed slot layout at a caller-chosen base, and `dk_fire(role, midi, vel)` (pitch is a first-class param — pass 0 for the role default; bend later without a refactor). Each kit's `build(base)` callback owns the *sound*, so distinct machines stay distinct (808≠909). `dk_use(&def, base)`. Built-in `DK_ELECTRO` / `DK_ACOUSTIC` | a cart that needs a **drum kit** to play or sample — a kit is a pad **MAP** (N different sounds, one per key at fixed pitch), the opposite of `keybed.h` (which pitches ONE voice), so drums live on the pad row, not the keybed. **Retrofit candidates** (not yet migrated — share the trigger, keep their sound): `tr808`/`tr909`/`drummachine`/`fingerdrums`, the radio stations, `notebass`/`onenotebass`, `omnichord`, `chordblossom`. NOT owned yet: choke groups (blocked on a per-slot note-off). See [mic-and-sampling.md](../design/mic-and-sampling.md) | `sampler` (drum sources) |
