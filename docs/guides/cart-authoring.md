...
| `cart_ctx.h` | the seam that lets a cart-land header's state be **PER-INSTANCE**: declare the header's state once as an X-list, then `DE_CTX_STATICS(LIST)` (the DEFAULT — exactly the file-scope statics that were there) or `DE_CTX_BLOCK(...)` when a cart defines `DE_CART_CTX` (the same list as a struct, reached through `de_state_for` so each engine instance gets its own) | **only if you are WRITING a cart-land header that holds state.** Cart authors never touch it: the default expansion is what every cart compiles, and it costs nothing. It exists because an AUv3 loads the same cart TWICE in one process, and a cart is ONE translation unit — so two racks would share a widget table or a drum tuning. Recipe + the four generation traps: [engine-context.md](../design/engine-context.md) → "Doing the next header" | all 8 stateful headers use it: `ui`, `keybed`, `solo`, `gestures`, `radio`, `tr808`, `cursor`, `drumkit`/`tr909` |
...
| `pro.h` | the **in-app-purchase gate**: `pro_unlocked()` (the gate), `pro_for_sale()` / `pro_can_purchase()` (is there a store here, can THIS process show a sheet), `pro_buy()` / `pro_restore()`, and one **shared Pro sheet** so three apps do not draw three paywalls. Stateless: the `ProSheet` belongs to the cart. A cart never names a product id, that comes from the app manifest through a generated `app_pro.h`, so the same cart runs inside a paid app, in the free web gallery and in the editor. `pro_module_*` is the per-rack CONTENT axis (Tiny Jam's launcher). ⚠ **fails OPEN** where no store is linked, which is right for the editor and web and is why `tools/pro-check.js` exists | a cart in a **shipping app** that has to put something behind the paid unlock (ADR-0035: WAV export, MIDI, the AUv3) | `tinyjam-menu` (the content axis); the feature axis lands with WAV export |
...
| `cards.h` | shared **playing-card rendering**: `card_suit(suit, cx, cy, col, s)` draws one suit pip centred, scaled from the canonical large pip by `s` (`s = 1.0` reproduces the blackjack/poker size pixel-exact). Suits 0–3 = heart/diamond/club/spade; the colour convention stays the cart's call | any **card game**: wrap it once with your card size (`draw_suit()`) instead of re-drawing four pips per cart. Include after `studio.h` | `blackjack`, `poker`, `solitaire`, `strippoker` |
...
| `roadkit.h` | the shared **at-grade junction geometry** (pure, param-driven): `curb_return()` (the tangent-arc curb fillet), `edge_corner()` (where two arms' gap-facing band edges cross), `rk_count_corners(brg, n)`, `rk_cross_hw()` (typed cross-section half-width), plus the N-arm-native `RkField` (`rk_field_build`/`rk_field_road` = arm-union ∪ fillets ∪ disc, space-agnostic) | a cart that has to **draw or query a junction**. A junction is N arms at bearings, no 90°/collinear assumption, and the caller supplies the bearing array, so `streetlab`'s leg model, OSM, and the generated city all feed the one impl. `streetlab` is the spec-locked source (scans in screen px); `citydrive` calls it in ground metres. Grade dispatch (`roadlab` interchanges) is WIP. See [worldgen-plan.md](../design/worldgen-plan.md) Track B | `streetlab` (source), `citydrive`, `citygrow`, `roadlab` |
| `citygen.h` | the calibrated procedural **city grammar** (worldgen rungs 2 to 5): `density_at()` (the population field), `ar_build()` (per-city tensor-field arterials, Chen 2008), `ar_graph`/`ar_faces`/`ms_build` (districts + minor-street fill), and `citygen_road_at()` (the drive seam). SNDi-matched to real Rotterdam, so `sndi-check --compare` passes | a cart that wants a **generated city** rather than a hand-drawn one. Pure over `worldnet.h`'s terrain/lattice/hash + `seedZ` + the `K_*`/`MS_*` knobs; a city is bounded, so generate one wholesale and the CALLER caches it (state here is file-static, one city at a time). `citygrow` stays the tuning bench. Open: reconciling its world model with `worldnet.h` so spine and city are one. See [worldgen-plan.md](../design/worldgen-plan.md) | `citygrow` (bench), `sloop` (drives it, press M) |
...
| `demath.h` | **deterministic float math — the same BITS on arm64 / x86-64 / wasm**: `de_sinf`/`de_cosf`/`de_tanf`/`de_atan2f`/`de_asinf`/`de_acosf`/`de_expf`/`de_logf`/`de_powf`/`de_tanhf`/`de_hypotf`, plus **`de_sin_turns`/`de_cos_turns`** which take **turns** (0..1) instead of radians — the phase-accumulator form, so the range reduction is exact and it runs **45% faster than libm's `sinf`**. Auto-included by `studio.h`; nothing to add. | a cart whose **output gets compared**: it carries a `spec()` oracle, records/replays input, wants lockstep netplay, or is byte-locked to a golden image. IEEE 754 mandates `+ - * /` and `sqrt` correctly rounded but says **nothing** about the transcendentals, so every platform's libm rounds `sin`/`exp`/`pow` differently — that is silent cross-device drift. **Decoration doesn't need this** (a title-screen wobble can stay on `sinf`). And `sqrtf`/`fabsf`/`floorf`/`fmodf`/`fminf`/`fmaxf` are **already bit-exact by spec** — there is no `de_` version and you don't want one. Pairs with the `FP_CONTRACT` pragma in `studio.h`; both are required, neither alone works. See [determinism.md](../design/determinism.md) | the whole audio engine (`sound.h`, zero libm left), `streetlab`, `citydrive`, `sloop` |
...
| `boxrig.h` | the shared **"sprite region → ≤8-vert Box2D polygon + texture"** toolkit (the puppetmaker representation, factored out of `puppet`/`boxlab`/`boxjelly` so they stop drifting): `boxrig_hull()` (sprite alpha → a `b2Hull` inside Box2D's 8-vertex cap), `boxrig_draw()` (texture the polygon via `tritex` from its OWN vertices, so the paint exactly covers the hull), `boxrig_point_in_body()` (click-to-grab), `boxrig_resolve_box`/`_poly()` (verlet↔rigid coupling, only when `physics.h` is included) | a cart making **textured rigid bodies out of sprite art**. Owns the pixels↔geometry math and the skin; the CART still owns body/shape/joint policy (density, friction, static vs dynamic, what joins to what) and the reaction impulses. Include `studio.h` then `box2d/box2d.h` first, and build the lib with `tools/build-box2d.sh`. Box2D is metres + y-up while `studio.h` is pixels + y-down, so every fn takes `ppm` + `screenH` and reads no cart global. See [box2d-integration.md](../design/box2d-integration.md) | `puppet`, `boxlab`, `boxjelly` |
| `mono.h` | **monosynth KEY ASSIGN** — the two questions a one-voice keyboard has to answer, which the engine has no opinion about: **which** held note sounds (`MONO_LAST`/`LOW`/`HIGH`/`FIRST` — Reid's four priority schemes) and whether a press **restarts the envelope** (`MONO_SINGLE` = only onto an empty keyboard, the Minimoog; `MONO_MULTI` = every press, the ARP; `MONO_ANY` = any transition, including a release that hands the voice over). One `Mono` struct owns the held-key stack; `mono_press`/`mono_release` return `MONO_START`/`GLIDE`/`RETRIG`/`STOP` and the cart decides how to voice each | a **monosynth** — `tb303`, `acidrack`, `moog` and `sh101` each hand-rolled one point in Reid's space of "at least 24 keyboard characteristics", so they silently disagreed about the behaviour that most defines how a monosynth answers the fingers. Pure logic: no engine surface, no UI, no sound — which is why it carries its **own** `mono_selfcheck()` (Part 18's four-priority table, run by the including cart's `spec()`). NOT `keybed.h` (the keyboard *widget*) and NOT `solo.h` (the radio scale strip); this is the decision layer between a key press and a note. See [synth-secrets-plan.md](../design/synth-secrets-plan.md) item 2.2 | `sh101` (PRIO/TRIG switches on the panel) |
...
| `morphdrum.h` | the shared **morphing drum voice bank**, the honest core behind the "supergroovebox" idea: an 808 kick and a 909 kick are not two engines, they are the SAME synthesis structure (sine body + pitch-env punch + lowpass + drive; snare = two sine modes + highpassed noise; hat = one bright ringing metal source) at different parameter values. So each voice is ONE parametric synth with a deep knob panel: `morph_build`/`morph_ride` (rebuild only the voices whose knobs moved)/`morph_fire`. `MD_CHARACTER` (0 = 808 … 1 = 909) morphs the *structural* voicing (the bits with no knob of their own: click brightness, noise-mode shape, FM timbre floor); every other knob is an absolute control whose range reaches well PAST both machines (deep sub, endless boom, more grit). Params are floats 0..1 indexed by `MD_*`; each voice reads the subset in `MD_PANEL[]`, which the cart iterates to lay out its knobs | a drum cart that wants the **parameter space between and beyond** the two classic machines, one knob panel instead of two banks. NOT `tr808.h`/`tr909.h` (two fixed faithful recipes) and NOT `drumkit.h` (a pad map) | `morphbox` |
| `sideman.h` | the shared **Wurlitzer Side Man (1959) voice bank**: ten vacuum-tube percussion circuits from the first drum machine ever sold. `sideman_build(base)` = the 12-slot bank, `sideman_fire(base, role, boost, delay)` = the trigger, `SIDEMAN_V[]` = the voice table (pitch/vol/gate/click) so the whole bank can be re-voiced in one place. `SM_*` 10-voice roster: BASS DRUM, TOM TOM I+II, WOOD BLOCK, TEMPLE BLOCK I+II, CLAVES, BRUSH, MARACAS, CYMBAL | a cart that wants the **pre-transistor** drum sound: four of the ten voices are struck WOOD (one damped ring through a resonant band, no noise, no layers, hard front, fast clean decay), the fullness comes from `DRIVE_ASYM` on every voice (a single-ended tube stage brings even harmonics) plus a ~60Hz-6kHz band limit, and the bass drum is soft and round because nobody had invented a kick that hits yet. It deliberately does NOT own the cabinet: the real unit fed the organ's amp, so the cart pins that stage itself with `outboard.h` | `sideman` |
| `rhythmbox.h` | **GENERATED** (`node tools/gen-rhythmbox.js`, source of truth = [rhythm-box-patterns.md](../design/rhythm-box-patterns.md)): **101 preset rhythms read off four manufacturers' own documents, including CHORD and BASS lanes** — `RB_FR2L[16]` (Ace Tone Rhythm Ace FR-2L, c.1969, whose rhythm units Ace Tone sold Hammond to build INTO organs), `RB_TR77[18]` (Roland TR-77, 1972) and `RB_SGS[30]` (SGS M252 LSI, both published factory masks, the chip that put AUTO RHYTHM inside cheap Italian organs) and `RB_M253[12]` (its sibling's mask AA; the sibling's other mask duplicates an M252 one byte for byte and is excluded, so the library carries no duplicates). A lane is a 48-bit **mask over counts**, so the library is a few KB of rodata; read it with `rb_hit`/`rb_uncertain`/`rb_used`/`rb_beat_of` | a rhythm cart that wants **real** period patterns instead of invented ones. THREE rules or it plays nonsense: `per_beat` and `bars` are **per rhythm** (the FR-2L waltz divides its 24-count bar by THREE, its slow rock reads all 48 counts as ONE 12/8 bar), `unused` marks counts a rhythm **skips** (the TR-77 hatches them on the page), and `RB_GATE` lanes are **held, not struck** (a brush swish, a guiro). FR-2L lane LABELS are provisional (75 dpi scan, the letterforms are interpolation): lane ORDER is reliable, so assign voices by order and never claim which voice plays which lane. NOT a voice bank — it is PATTERNS only, so pair it with `sideman.h`/`tr808.h`/`drumkit.h` for sound | none yet (the cart is the open item; `sideman`'s twelve rhythms are reconstructions by contrast) |
| `drumpat.h` | **GENERATED** (`node tools/gen-drumpat.js`): **565 modern 16-step patterns in 92 named groups** (CR78, the classic-genre set, then rock/funk/disco/reggae/house/hiphop/dnb/dubstep/EDM/breaks and more), bit-packed one `uint16` per role per pattern so the library is 36 KB of rodata. Read with `dp_hit`/`dp_flam`/`dp_accent`/`dp_role_used`. `DP_AC` is a **velocity lane, not a drum**, and some steps are **flams** (a hit that wants a doubled stroke) | a drum cart that wants a **big vocabulary of modern grooves** on a plain 16-step grid. NOT `rhythmbox.h`: that one is period hardware on the machines' own clocks (24 counts a bar, six per beat, held gates, skipped states, two-bar claves), none of which 16 sixteenths can express. Provenance is uneven and the header says so per group | `groovebook` (the browser: 92 groups, letter-jump, plays while you scrub) |
...
| `outboard.h` | the `ampcab.h` move for the **master bus**: the four-stage **analog outboard chain** as one voicing table (EQ = `eq_inst(0)` · IRON = `drive_insert(DRIVE_ASYM)`, the odd+even shaper · COMP = an `eq_inst(1)` INPUT boost feeding `glue()`, because a FET unit has no threshold and is driven by its input · PLATE = `reverb()` + per-slot sends). Bundles, not new DSP | a cart or app that wants **"the analog output chain"** on the mix rather than hand-tuning eq/drive/glue per cart. Carries two measured facts so you do not re-derive them: the comp is **pinned after the inserts** (only EQ→IRON→COMP is achievable), and you must **meter RMS on the comp, not peak** (at the hardest ratio `glue` takes 6.3 dB off the crest and gives +2.3 dB of RMS back; it behaved the opposite way before it gained makeup on 2026-08-19, so older prose about it is stale). ⚠ it **owns** the master insert order. See [analog-outboard-chain.md](../design/analog-outboard-chain.md) | `outboard` |
| `pmbreed.h` | the cart-land face of `tools/patch-match/pmpatch.h`: `pm_mutate()` perturbs a `PmPatch` in the same 0..1 space the search walks (snapped macros step a measured detent, they do not nudge between them) and `pm_apply_slot()` writes the whole patch onto a slot, leftovers cleared | a cart that **breeds patches by ear** — hear a set, keep one, refill the rest with siblings. No scoring, no second engine instance. See [patch-matching-cart.md](../design/patch-matching-cart.md) option B | `patchbench` |
