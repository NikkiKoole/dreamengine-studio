...
| `fill`/`outline`/`dither` of a shape | the `raster_test` **cart** (`tools/raster_test.script`) | fill, outline, dither, solid all agree pixel-for-pixel ([`rasterization-consistency.md`](../design/rasterization-consistency.md)) |
