...
| **Patch match** — drop a `.wav` → spawn `pm` → eight candidates to audition, then **open the whole set in the `patchbench` cart** (it writes `de:patch-slots` and runs it) or **copy block** one to the clipboard. On the bench: pick a pad, **BREED** mutates around the keep, **UNDO** walks back. A **run browser** lists earlier searches with their best loss. Shift-drop = `--quick`, which is faster and hands back **six** (it refines 3 engines × 2 seeds instead of × 3). Desktop only (it is a tool spawn, not an in-cart render; ADR-0006) | drop on the window | [`../design/patch-matching-cart.md`](../design/patch-matching-cart.md) options A + B |
...
| **✨ Dress a clip** — a baked clip → a 9:16 Short with **hand-typed text** in the letterbox bars, drawn in the **real engine `dos_8x8` pixel font** with **boil** + a **tween-in** entrance (via the `titlecard` cart). A layout preview updates as you type; **▶ preview motion** bakes a short real clip; boil/breathe sliders + entrance dropdown; Dress renders `<label>-dressed.webm` | Promote §A "✨ dress" on a baked row | [`../design/export-ratios.md`](../design/export-ratios.md) "Dressed composite"; drives `tools/dress-clip.js` |
...
- **`tools/dress-clip.js`** — the CLI behind the Promote **✨ dress** button (above): dress a baked clip into a 9:16 Short with **hand-typed text** in the letterbox bars, drawn in the **real engine pixel font** with boil + a tween-in entrance (bakes two `titlecard` cards + colour-keys them in). The "add text" step of record → bake → **dress** → post. `--preview <png>` = the fast drawtext layout still; `--secs <n>` = a short motion preview; `--boil/--breathe/--anim` = the kinetic knobs. [`../design/export-ratios.md`](../design/export-ratios.md) "Dressed composite".
