...
| **Patch match** — drop a `.wav` → spawn `pm` → eight candidates to audition + paste an `instrument()` block into the open buffer. Shift-drop = `--quick`. Desktop only (it is a tool spawn, not an in-cart render; ADR-0006) | drop on the window | [`../design/patch-matching-cart.md`](../design/patch-matching-cart.md) option A |
