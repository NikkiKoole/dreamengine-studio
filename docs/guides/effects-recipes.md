...
| ...and give the word PAUSES | put REST slots in the word; on a rest duck the wet `mix` toward 0 with an **asymmetric** gate (snap shut ~0.55/frame, ease open on the glide rate) | **the step that turns a slur into speech.** Measured on `acidcandy`: a gapless vowel-per-note word moved the spectrum no more than a *static* vowel (centroid std 1270 vs 1281 Hz), because the phrase had no boundary. Adding two rests to an 8-slot word took std to 1964 Hz (+55%). Symmetric gating does **not** work: a pause lasts one note (~6.8 frames for a 16th at 132 BPM) and an eased close needs ~7 frames just to half-shut, so the mouth never closes | `acidcandy` (SPK) |
| ...with pause depth on a knob | one float scaling the rest's gate target (`gate_target = 1 - gap`) | a continuous slur→breathe→full-stop axis, and every position stays musical. Measured: gap 0.0 / 0.5 / 1.0 → centroid std 1290 / 1597 / 1964 Hz, monotonic, with 0.0 reproducing the gapless version exactly (so the knob A/Bs the whole idea from inside the instrument) | `acidcandy` (GAP) |
| ...and pause **density** without going random | keep a parallel `REST_AT[]` threshold per word slot; a slot rests once the knob reaches its threshold | **the way to make a fixed structure parametric without randomising it.** Rests arrive in a fixed hand-authored order (phrase-end first, then the mid-phrase breath), so every knob position is deterministic and repeatable, unlike a per-note rest *chance* which trades the word for a stutter. Put unreachable thresholds (>1) on the strong syllables so they can never drop out. Measured: dens 0 / .25 / .5 / .7 / .95 → resting slots {} / {7} / {3,7} / {3,5,7} / {1,3,5,7}, std 1275→2457 Hz. Set the default threshold set so it reproduces the previous hard-coded word **byte-identically** — that is the gate proving the knob added a control without moving the sound | `acidcandy` (DENS) |
