...
| ...with pause depth on a knob | one float scaling the rest's gate target (`gate_target = 1 - gap`) | a continuous slur→breathe→full-stop axis, and every position stays musical. Measured: gap 0.0 / 0.5 / 1.0 → centroid std 1290 / 1597 / 1964 Hz, monotonic, with 0.0 reproducing the gapless version exactly (so the knob A/Bs the whole idea from inside the instrument). Prefer this to randomising rest *density*, which trades the word for a stutter | `acidcandy` (GAP) |
