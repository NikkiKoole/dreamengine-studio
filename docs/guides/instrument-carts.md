...
| **one note** (`onenote`) | A one-note funk groovebox (after BeatCraft Studio's "magic of one note") — kick/snare + TWO bass rows that play a *single* note each; the song comes from RHYTHM + live, scale-locked TRANSPOSE knobs (ride T1/T2 to slide each loop up the minor pentatonic), −1-oct SUB toggles, and a global ride FILTER. The "no melody, just move the one note" idea. | `drummachine` step-grid chassis + `ui.h` knobs, `INSTR_SAW` bass |
