...
|---|---|---|---|
| **A single-engine showcase** ("here's what INSTR_X sounds like") | one `INSTR_*` engine + a simple keyboard/pad | `pluck`, `mallet`, `organ`, `epiano`, `fm`, `pd`, `tabla`, `handpan`, `modal` | The tech-demo pattern: one engine, a few live knobs, an autoplay noodle. `modal` is the §7.1 exciter→resonator (A/B mapping toggle). ~250–390 lines each. |
...
lip-reed brass (trumpet→tuba, the slide horn) · `MODAL` = exciter into resonator (strike /
...
| **cloud hold** (`cloudhold`) | Boutique freeze pedal — hold a chord, stomp FREEZE, play over the pad cloud, unfreeze. The product cart for `grains_freeze` (compound-blindspot §6 / #29): freeze is the gesture, not a lab knob. Cousin of `grains` (the texture toy) and `grainchop` (the sampler spike). CLOUD / GLASS / DUST retune grain size/density/scatter/pitch-spread (same capture, different room). SHIMMER = +12 `grains_pitch`. | `face.h`, `keybed.h`, `ui.h`, `INSTR_SAW`, `instrument_grains`/`_freeze`/`_pitch` |
| **live loop** (`liveloop`) | Phone-first live looper — freeze a bar of mic (hold/clap/sing/guitar) into a looping layer, stack another take, mute/clear. Capture-then-freeze (`mic_record` → `sample_load` → `SAMPLE_LOOP`) under live `input_monitor` (the micfuzz / GUITAR IN path). Overdub is a new layer, not a circulating PCM ring. loopstation chrome, audio-input-frontier ★1 / #17. | `face.h`, `ui.h`, `input_monitor`, `mic_record`/`sample_load`, `INSTR_SAMPLE` + `SAMPLE_LOOP` |
...
| **chordblossom** (`chordblossom`) | A truthful homage to the Telepathic Instruments Orchid (ORC-1, 2026) — you play CHORDS not notes: a one-octave keybed picks the root, four TYPE buttons (dim/min/maj/sus) + four combinable MODIFIER buttons (6/m7/maj7/9) build the quality, and the signature VOICING dial cascades the chord one chord-tone at a time through inversions + octave spreads. Three engines, following sub-bass, reverb+chorus, strum/harp/arp/pattern/slop, a sonic-strings plate, a drum machine + a real-time chord looper. The Orchid's keybed-root + type/modifier logic is distinct from `omnichord`/Strumharpy's chord-grid Suzuki tribute. | `INSTR_SAW`/`INSTR_FM`/`INSTR_EPIANO`, `strum`/`schedule_hit`, `reverb`/`chorus`, `ui.h` knobs |
...
| **loopstation** (`loopstation`) | A live-looper pedal — arm a track, play, layer; the first cart that records itself (event/CC level — not PCM). | beat clock, `note_on`, all chiptune waves |
