...
// DIFFERENT parameter values. So each voice is ONE parametric synth with a
...
static void md__resolve(const MorphKit *k, int v, MDRes *r) {
    ...
    case MD_PLUCK: {  // a MELODIC voice — CHAR morphs dark↔bright material (not 808↔909)
        ...
        r->midi = md_penta(p[MD_TUNE], 33) + oct * 12;       // pentatonic pitch + octave (SUB)
        r->dec  = md_lin(p[MD_DECAY], 60, 1400);             // note length
        r->harm = 0.30f + p[MD_DECAY] * 0.65f;               // PLUCK harmonics = ring time (~decay)
        r->timbre = MD_L(0.25f, 0.90f, ch);                  // CHAR: dark round → bright metallic
        r->morph  = p[MD_TONE];                              // TONE: the 3rd pluck macro
        r->cut  = md_exp(p[MD_CUT], 200, 5.0f);              // 200..6400
        r->res  = md_lin(p[MD_RES], 0, 10);
        r->drv  = p[MD_DRIVE];
        break;
    }
    }
...
static void morph_apply(MorphKit *k, int v) {
    ...
    case MD_HAT:
        ...
        break;
    case MD_PLUCK: {
        int atk = md_lin(k->p[v][MD_PUNCH], 0, 25);          // ATK: pluck attack softness
        instrument(b + MDS_PLK, INSTR_PLUCK, atk, r.dec, 0, 40);
        instrument_harmonics(b + MDS_PLK, r.harm);
        instrument_timbre(b + MDS_PLK, r.timbre);
        instrument_morph(b + MDS_PLK, r.morph);
        instrument_filter(b + MDS_PLK, FILTER_LOW, r.cut, r.res);
