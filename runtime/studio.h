...
#define INSTR_PIPE    25  // blown flute — flute/recorder/pan pipe. A self-oscillating STK jet-drive bore; holds while sustained, breathy like a real flute. macros: harmonics = jet drive (brighter, and pulls flat — see below), timbre = breath air (pure → airy), morph = embouchure (hollow/dark → focused/bright). ⚠ MEASURED 2026-07-28 (Synth Secrets audit §K, plan item 0.13): the bore is CLOSED — evens sit 50–73dB under the odds, so it is spectrally a PAN PIPE, not an open flute/recorder (those have even harmonics). ⚠ `harmonics` does NOT overblow: no register jump at any value, it drives the jet nonlinearity so the tone brightens and goes FLAT (−26¢ by C5). (A closed pipe would overblow a TWELFTH, not the octave the old docstring promised.) ⚠ TUNING tracks morph (embouchure) — in tune at the showcase flute m0.70 (worst −1.5¢), but NON-MONOTONIC away from it: morph 0.40 is +13…+17¢ SHARP and morph 0.20 is −19¢ then unstable at the top. VERIFY a new voice's register: node tools/tune-check.js --engine PIPE --macros h,t,m --range lo-hi
...
#define INSTR_MODAL   31  // exciter into resonator — a bank of tuned FILTERS you strike / blow / bow into (not decaying sines). One id covers mallet-gaps, breath/noise, and additive-ish pads because a filter bank has an input. macros (recommended mapping): harmonics = GEOMETRY (smooth: plate→string→bar/tube→bell/bowl), timbre = MATERIAL (bright+ringing → dull+short), morph = EXCITER mix (strike→blow→bow). The Elements mapping (timbre=brightness, morph=damping) is a live MODE_MODAL_MAP toggle — both routes are hearable from one build. MODE_MODAL_POS / DIRECT / MODES / EXCITE sit on the aux channel. Try the “modal” cart.
#define INSTR_FM4     32  // four-operator FM — TX81Z/OPN/OPM/OPZ eight-algorithm family (not a free matrix, not a DX7). Phase modulation of the accumulator, so operator feedback stays stable (the 2-op already did this). macros (recommended mapping): harmonics = VOICING (snapped patch: algorithm + 4 ratios + 4 levels — tine → bell → metal → brass → bass → wood → glass → organ), timbre = brightness (master index on every modulator), morph = feedback (clean → growl → clang). The alt mapping (harmonics = algorithm, ratios via MODE_FM4_R0..R3) is a live MODE_FM4_MAP toggle — both routes are hearable from one build. Bell voicings carry √2 / φ (the shipped 2-op table cannot). Try the “fm4op” cart.
...
void sample_autotune(int slot, int root, int scale, float amount);  // AUTO-TUNE a recorded take in `slot` onto a key (root pitch-class 0..11) + SCALE_*: pitch snaps to the scale and de-wobbles while the VOICE/timbre is kept (formant-preserving, not a chipmunk). amount 0..1 (0=off, 1=full snap). Offline, in place — record → mic_record_read → sample_load → sample_autotune → play. docs/design/transparent-autotune.md
void sample_shift(int slot, float semitones);        // TRANSPOSE a recorded take in `slot` by semitones (-24..24), KEEPING ITS LENGTH — the pitched-up ad-lib / chipmunk stack (playing a sample slot at a higher note shortens it; this doesn't). Formants ride along with the pitch, so it sounds pitched, not re-voiced. Offline + in place, like sample_autotune. docs/design/transparent-autotune.md
...
void instrument_harmonics(int slot, float x);  // engine macro 1 of 3 — e.g. PLUCK ring time · MALLET bar material · FM carrier:mod ratio · GUITAR body · PIANO voicing · BRASS instrument/bore (trumpet→tuba) · MODAL geometry (plate→string→bar→bell) · FM4 voicing (or algorithm, if MODE_FM4_MAP is the alt route)
void instrument_timbre(int slot, float x);     // engine macro 2 of 3 — e.g. PLUCK pick brightness · MALLET mallet hardness · FM brightness · GUITAR string brightness · PIANO hammer · BRASS brassiness · MODAL material (or brightness, if MODE_MODAL_MAP is the Elements route) · FM4 brightness (master modulator index)
void instrument_morph(int slot, float x);      // engine macro 3 of 3 — e.g. PLUCK pick position · MALLET ring length · FM feedback growl · GUITAR mute · PIANO pedal · BRASS breath/lip lean-in · MODAL exciter mix (or damping, if MODE_MODAL_MAP is the Elements route) · FM4 feedback (clean → growl → clang)
...
void instrument_mode(int slot, int idx, float value);  // per-engine aux channel, NOTE-ON face — a slot's structure/mode params past the 3 macros, read at the next note_on. idx is per-engine; use the MODE_* names. GUITAR/PIANO: MODE_STRING_WEIGHT/MODE_STRING_CLICK. PIANO also: MODE_PIANO_DECAY/MODE_PIANO_KNOCK/MODE_PIANO_STRETCH/MODE_PIANO_STIFF. BOWED: MODE_BOW_PIZZ (>=0.5 plucks instead of bows) / MODE_BOW_BODY / MODE_BOW_SIZE. ORGAN: MODE_ORGAN_PERC_THIRD/MODE_ORGAN_PERC_SLOW/MODE_ORGAN_LEAK. MODAL: MODE_MODAL_POS/DIRECT/MODES/EXCITE/MAP. FM4: MODE_FM4_ALG/R0..R3/MAP. value 0..1. (decision 0017)
...
#define MODE_PIANO_STIFF   5   // instrument_mode idx — INSTR_PIANO: STIFF-STRING inharmonicity (a real piano's partials stretch SHARP — the metallic shimmer in the attack, and the clang in the bass). 0 = a perfectly harmonic string, 0.5 = the voicing's own amount, 1 = double. Most audible in the bass and in chords
...
#define MODE_MODAL_POS     0   // instrument_mode idx — INSTR_MODAL: where on the surface it is excited (0 = center / low-mode thump → 1 = edge / high-mode ring)
#define MODE_MODAL_DIRECT  1   // instrument_mode idx — INSTR_MODAL: how much RAW exciter you hear (STK directGain / Elements strike-bleed). 0 = all resonator, 1 = all attack bite
#define MODE_MODAL_MODES   2   // instrument_mode idx — INSTR_MODAL: mode-count BUDGET 0..1 → 4..12 resonators. 4 is the floor (STK ModalBar); more is a voicing choice, not a constant
#define MODE_MODAL_EXCITE  3   // instrument_mode idx — INSTR_MODAL: Elements-route exciter mix (0 strike → 0.5 blow → 1 bow). Ignored on the recommended mapping, where morph IS the exciter
#define MODE_MODAL_MAP     6   // instrument_mode idx — INSTR_MODAL: which three-macro mapping is live. <0.5 = recommended (morph=exciter, timbre=material). >=0.5 = Elements (timbre=brightness, morph=damping, exciter from MODE_MODAL_EXCITE). A RUNTIME fork, not a #define — the showcase cart toggles it so both routes are hearable from one build
#define MODE_FM4_ALG       0   // instrument_mode idx — INSTR_FM4: algorithm 0..1 → the eight OPN/OPM/OPZ routings. Live on the alt mapping (where harmonics IS the algorithm). On the recommended mapping the voicing bakes this
#define MODE_FM4_R0        1   // instrument_mode idx — INSTR_FM4: operator 1 ratio (snapped, includes √2 / φ). Alt mapping; ignored on recommended (voicing bakes the four ratios)
#define MODE_FM4_R1        2   // instrument_mode idx — INSTR_FM4: operator 2 ratio
#define MODE_FM4_R2        3   // instrument_mode idx — INSTR_FM4: operator 3 ratio
#define MODE_FM4_R3        4   // instrument_mode idx — INSTR_FM4: operator 4 ratio
#define MODE_FM4_MAP       6   // instrument_mode idx — INSTR_FM4: which three-macro mapping is live. <0.5 = recommended (harmonics=voicing). >=0.5 = alt (harmonics=algorithm, ratios from MODE_FM4_R0..R3). A RUNTIME fork, not a #define — the showcase cart toggles it so both routes are hearable from one build
...
void instrument_unison(int slot, int n_voices, float detune);  // UNISON: sum `voices` (1..7) detuned copies of the slot's wave = the supersaw wall. detune = spread in semitones (0.1 shimmer .. 0.7 wide). voices applies at the next note; detune rides LIVE. wavetable slots only (saw/square/tri/sine). 1 = off (default)
...
void autotune_mic(int root, int scale, float amount); // LIVE auto-tune: sing and hear yourself corrected in real time — pitch snaps to (root pitch-class 0..11, SCALE_*) while your VOICE is kept (formant-preserving), monitored into the mix. amount 0..1 (0 = off). Needs mic_start(). LIVE (breaks .rec replay — decision 0032). The streaming twin of sample_autotune
void harmonize_mic(float semitones, int n_voices);  // LIVE HARMONISER: sing and hear yourself transposed in real time — the fixed-interval twin of autotune_mic (one corrector, so the last call wins). semitones -24..24, voices 1..3 = a STACK (the shift, + a fifth, + an octave) from your one voice. Formants ride the pitch (chipmunk-flavoured, not a re-voicing). Needs mic_start(). LIVE (breaks .rec replay — decision 0032)
void input_monitor(float gain);                       // route the LIVE MIC through the MASTER fx chain (fx_order/crush/filter/reverb/…) — hear your OWN sound processed in real time (guitar/voice → the pedalboard). gain: 0 = off, ~1 = unity. Needs mic_start() + permission. LIVE (breaks .rec replay — decision 0032). audio-input-frontier.md ★1 the pedal tier
...
void *de_state_for(const void *key, int bytes);  // per-instance state for ONE cart-land header — pass the address of a file-scope sentinel of your own as `key` (unique per translation unit). Carves a slice out of the same block de_state() owns, so two plug-in instances of one cart get their own. Never cache the pointer: registering another key can move it. SCRATCH: not written to session state (see de_state_for_saved).
void *de_state_for_saved(const void *key, int bytes);  // like de_state_for, but this slice IS written to session state (de_save_state) and restored — use it for what the PLAYER chose (knobs, patterns, tuning). Everything a fresh init() would rebuild stays on de_state_for. NEVER put a live voice handle, a pointer, or per-frame geometry in a saved slice: it is restored into a DIFFERENT instance where those mean nothing.
