...
#define MODE_BOW_BODY      1   // instrument_mode idx — INSTR_BOWED: BODY resonance amount. A violin's body is what makes it a violin rather than a sawtooth, and this engine shipped without one. 0 = no body (the bare string, as before), 1 = all body. EXPERIMENTAL (Synth Secrets §M2)
#define MODE_BOW_BODYTYPE  2   // instrument_mode idx — INSTR_BOWED: which body model. <= 0.5 = three short delay lines (Reid's "a body is a small reverberant room", dense + physical; this is the default, since index 2's bank default of 0.5 is inherited from PIANO), > 0.5 = four bandpass formants (fewer resonances, more contrast). EXPERIMENTAL — here to be A/B'd; the loser gets removed
...
void harmonize_mic(float semitones, int voices);  // LIVE HARMONISER: sing and hear yourself transposed in real time — the fixed-interval twin of autotune_mic (one corrector, so the last call wins). semitones -24..24, voices 1..3 = a STACK (the shift, + a fifth, + an octave) from your one voice. Formants ride the pitch (chipmunk-flavoured, not a re-voicing). Needs mic_start(). LIVE (breaks .rec replay — decision 0032)
