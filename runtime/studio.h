...
void instrument_harmonics(int slot, float x);  // engine macro 1 of 3 — e.g. PLUCK ring time · MALLET bar material · FM carrier:mod ratio · GUITAR body · PIANO voicing · BRASS instrument/bore (trumpet→tuba) · MODAL geometry (plate→string→bar→bell)
void instrument_timbre(int slot, float x);     // engine macro 2 of 3 — e.g. PLUCK pick brightness · MALLET mallet hardness · FM brightness · GUITAR string brightness · PIANO hammer · BRASS brassiness · MODAL material (or brightness, if MODE_MODAL_MAP is the Elements route)
void instrument_morph(int slot, float x);      // engine macro 3 of 3 — e.g. PLUCK pick position · MALLET ring length · FM feedback growl · GUITAR mute · PIANO pedal · BRASS breath/lip lean-in · MODAL exciter mix (or damping, if MODE_MODAL_MAP is the Elements route)
