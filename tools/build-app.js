...
static int home_holding(void) {
    for (int i = 0; i < touch_count(); i++)          // any finger inside the top-left pad
        if (touch_x(i) >= 0 && touch_x(i) < HOME_HIT_W && touch_y(i) >= 0 && touch_y(i) < HOME_HIT_H) return 1;
    return mouse_down(MOUSE_LEFT)                     // mouse (desktop / editor bundle test)
        && mouse_x() < HOME_HIT_W && mouse_y() < HOME_HIT_H;
}
static void draw_home_chip(void) {
    ...
    if (home_hold > 0) {                              // fill left→right as you hold (progress)
        ...
        rectfill(HOME_X, HOME_Y, fw, HOME_H, CLR_INDIGO);
    }
    rect(HOME_X, HOME_Y, HOME_W, HOME_H, home_hold > 0 ? CLR_WHITE : CLR_LIGHT_GREY);
    for (int i = 0; i < 3; i++)                        // hamburger glyph = "back to the menu"
        line(HOME_X + 5, HOME_Y + 4 + i * 3, HOME_X + HOME_W - 5, HOME_Y + 4 + i * 3, CLR_LIGHT_GREY);
}
` : ''
