...
` : ''
// home chip (bundle-with-launcher only): a tap on the top-left chip returns to the
// overview — TAB doesn't exist on a phone. Drawn by the shim AFTER the cart, so a cart
// run standalone (no shim) never shows it. Cross-input: a lifted finger OR a mouse click.
const homeChip = launcher ? `
#define HOME_X 2
#define HOME_Y 2
#define HOME_W 18
#define HOME_H 14
static int home_tapped(void) {
    for (int i = 0; i < touch_ended_count(); i++) {   // touch: a finger lifted inside the chip = a tap
        int tx = touch_ended_x(i), ty = touch_ended_y(i);
        if (tx >= HOME_X && tx < HOME_X + HOME_W && ty >= HOME_Y && ty < HOME_Y + HOME_H) return 1;
    }
    return mouse_pressed(MOUSE_LEFT)                   // mouse (desktop / editor bundle test)
        && mouse_x() >= HOME_X && mouse_x() < HOME_X + HOME_W
        && mouse_y() >= HOME_Y && mouse_y() < HOME_Y + HOME_H;
}
static void draw_home_chip(void) {
    ...
    rectfill(HOME_X, HOME_Y, HOME_W, HOME_H, CLR_BLACK);
    rect(HOME_X, HOME_Y, HOME_W, HOME_H, CLR_LIGHT_GREY);
    for (int i = 0; i < 3; i++)                        // hamburger glyph = "back to the menu"
        line(HOME_X + 4, HOME_Y + 4 + i * 3, HOME_X + HOME_W - 5, HOME_Y + 4 + i * 3, CLR_LIGHT_GREY);
}
` : ''
...
void update(void) {
    ${tabLine}${launcher ? '\n    if (active != 0 && home_tapped()) { activate(0); return; }   // home chip → overview (consume the tap)' : ''}
