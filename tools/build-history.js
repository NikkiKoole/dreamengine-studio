...
// ---- the frontier: the present-tense open edge (docs/HANDOFF.md active lanes) ----
// The retrospect looks BACK at the whole arc; this shows what's mid-flight RIGHT NOW,
// read live from handoff.js's lane parser + the cart de:meta.todo[] debt. It self-
// refreshes and a landed lane just drops off — deliberately NOT woven into a dated
// past era (HANDOFF.md is a rolling, pruned file). See guides/history-page.md.
let frontier = null
try {
  const raw = execFileSync('node', [path.join(__dirname, 'handoff.js'), '--json'], { cwd: REPO, encoding: 'utf8' })
  const { lanes = [], staleDays = 14 } = JSON.parse(raw)
  const laneCards = lanes.map((l) => ({
    title: l.title, date: l.date, age: l.age, stale: l.age > staleDays,
    links: (l.links || []).map((rel) => {
      const meta = docMap[path.basename(rel)] || {}
      return { rel, name: path.basename(rel, '.md').replace(/-/g, ' '), desc: meta.desc || '' }
    }),
  }))
  // the polish debt the shipped surface hides — a NUMBER, never the list (that lives in cart-todos.js)
  let todo = null
  try {
    const t = execFileSync('node', [path.join(__dirname, 'cart-todos.js'), '--count'], { cwd: REPO, encoding: 'utf8' })
      .replace(/\x1b\[[0-9;]*m/g, '')
    const m = t.match(/(\d+)\s+open todo\(s\)\s+across\s+(\d+)\s+cart/i)
    if (m) todo = { open: +m[1], carts: +m[2] }
  } catch (e) { /* todos optional */ }
  if (laneCards.length) frontier = { lanes: laneCards, staleDays, todo }
} catch (e) { /* handoff optional */ }
...
/* the frontier — present-tense open edge (active handoff lanes), blue not pink */
section.frontier{margin-top:60px}
section.frontier .rs-head h2{background:#7ee0ff}
.rs.fr .rs-n{background:#7ee0ff;font-size:14px;padding:3px 9px;top:-12px}
.rs.fr .rs-n.stale{background:var(--orange)}
.rs.fr .tags{margin-top:10px}
.fr-debt{font-family:var(--mono);font-size:12px;color:var(--dim);margin-top:20px;padding-top:12px;border-top:2px dashed var(--panel2)}
.fr-debt b{color:var(--orange)}
`
...
function render(){
  ...
  if (DATA.frontier && DATA.frontier.lanes.length){
    const tw = $('div','toc-week')
    const head = $('a','tw-head'); head.href = '#frontier'
    const lab = $('span','tw-label', '⛏'); lab.style.background = '#7ee0ff'
    head.appendChild(lab)
    head.appendChild($('span','tw-tag', 'The frontier'))
    tw.appendChild(head)
    tw.appendChild($('div','tw-stats', '<b>'+DATA.frontier.lanes.length+'</b> active lanes · where it stands now'))
    toc.appendChild(tw)
  }
  ...
  // the frontier — the present-tense open edge (active handoff lanes), read off NOW
  if (DATA.frontier && DATA.frontier.lanes.length) root.appendChild(renderFrontier(DATA.frontier))
...
function renderFrontier(fr){
  const s = $('section','retrospect frontier'); s.id = 'frontier'
  const h = $('div','rs-head'); h.appendChild($('h2', null, 'The frontier — where it stands now'))
  s.appendChild(h)
  s.appendChild($('div','rs-lede',
    'The retrospect looks back at the whole arc; this is the open edge right now — the complex work still '+
    'mid-flight, read live from the active lanes in docs/HANDOFF.md. A lane drops off the moment its work lands.'))
  const grid = $('div','rs-grid')
  fr.lanes.forEach((l) => {
    const c = $('article','rs fr')
    c.appendChild($('div','rs-n'+(l.stale?' stale':''), l.age === 0 ? 'today' : l.age + 'd'))
    c.appendChild($('h3', null, esc(l.title)))
    if (l.links.length){
      const tags = $('div','tags')
      l.links.forEach((d) => {
        const tag = $('span','tag handoff', esc(d.name))
        tag.setAttribute('data-doc', d.rel)
        if (d.desc) tag.title = d.desc
        tags.appendChild(tag)
      })
  ...
  s.appendChild(grid)
  if (fr.todo){
    s.appendChild($('div','rs-lede fr-debt',
      'And the polish the shipped surface hides: <b>'+fr.todo.open+'</b> open todos across <b>'+fr.todo.carts+'</b> '+
      "carts (each cart's <code>de:meta.todo[]</code> punch-list — the full list lives in <code>cart-todos.js</code>, not here)."))
  }
  return s
}
