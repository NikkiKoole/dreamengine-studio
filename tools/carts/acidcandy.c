/* de:meta
{
  ...
  "todo": [
    "EXPORT BUTTON: where does it live, and what does it export? (OPEN, 2026-08-19 - a DESIGN question, not plumbing). The wiring is DONE and proven on a phone: runtime/pro.h + export_audio(name, seconds, format), reference cart `exportdemo`, docs/design/pro-unlock.md. Nothing here calls it, so ADR-0035's Pro unlock currently gates NOTHING in this app. The product rule is already decided: SHARE (EXPORT_M4A) is always available so a free player can post what they made, and only the lossless take asks for Pro - the wall is on the FORMAT, not the feature. What is NOT decided is this rack specifically. (a) WHERE: the face is already full; the master panel is the honest home, but it competes with PCF/crush/gate. (b) WHAT: this rack HAS a transport, so unlike pedalboard it can export an exact musical length - the current pattern, or the whole CHAIN when songmode is on, rather than 'the last N seconds'. Derive the seconds from plen/bpm and the chain length; the engine caps at 60s. (c) The take must START AT A DOWNBEAT or the file begins mid-bar, which no amount of fade fixes. `acidrack` already has a WAV button driving the DEBUG .bake/wav_request path - read it for the shape, but do NOT copy it: that path writes into the sandbox on iOS where nobody can reach it, which is exactly what export_audio + the share sheet replace. This cart's own description has wanted 'WAV export' since 2026-07-18.",
    "MIDI OUT CANNOT REACH THE HOST, and this rack is already the sequencer (OPEN, 2026-08-15). Found while surveying the AUv3 component types: everything in the MIDI OUT block is FINISHED work (303a ch1 / 303b ch2 / 808 GM ch10 / 909 GM ch11, slides as overlapping notes, accents as velocity, real drum gate lengths, 24ppqn clock, start/stop, a shutdown flush that never strands a held note) and it all goes to a CoreMIDI VIRTUAL SOURCE, which is the STANDALONE path. A plug-in's MIDI reaches its host through the AU's MIDIOutputEventBlock, and the engine has NO outward seam at all: ios/Sources/engine.h exports audio-in, notes, bend, CC and transport, and nothing the other way; the AU declares neither MIDIOutputEventBlock nor midiOutputNames (grepped, zero hits). So under a DAW the pattern leaves by a side door the project cannot record, route or save. THE CART SIDE IS DONE (mout_on already gates it, the pattern already streams) and the work is engine + Swift: a ring behind midi_send_* plus something like de_midi_out_drain(DeInstance*, DeMidiOut*, int) for the render block to pull. midi_output.h already tracks held notes per channel for its shutdown flush, so most of the bookkeeping exists. WHY IT MATTERS BEYOND A BUGFIX: with it this rack is also an `aumi` MIDI PROCESSOR that plays the user's OTHER plug-ins, which is near-zero CPU in the host (the constraint that kills every richer idea), competes on a much emptier shelf than the synth slot, and makes the PANEL the entire product. Open question in the design: does the ring REPLACE the virtual source in a plug-in build or run alongside it (an appex publishing a system-wide port is flagged uncovered). Survey: docs/design/auv3-plugin-types.md §3.2 / §4.2",
    "THE RACK IS DEAF: no host audio in (OPEN, 2026-08-15). The AU declares an explicitly EMPTY input bus array (TinyjamAU.swift:102), so nothing the host plays can reach the cart. The seam for it ALREADY EXISTS and is already exported: de_audio_input(const float*, int, int) in runtime/platform.h:215 and ios/Sources/engine.h, because the engine never opens a capture device by design and each host PUSHES frames in. An aufx/aumf render block hands over exactly that shape. If it lands, mic_level()/mic_pitch() make the visuals react to the host's track, input_monitor() puts the host through the master FX chain, and vocoder_mic/autotune_mic/harmonize_mic become host effects. Declare `aumf` (music effect: audio in + audio out + MIDI in) and NOT `aufx`, for the same wiring cost. THREE CAVEATS, so nobody budgets this as trivial: the mic ring was built for ANALYSIS and its latency/block alignment for a sample-exact insert is unmeasured; input_monitor is LIVE (ADR-0032) so no determinism gate covers it; and de_audio_input carries `seam-lint-ignore: one capture device per process`, reasoning that is WRONG for an effect since two instances on two tracks each have their own input. Note this is a shape for the ENGINE, probably not for this cart (a groovebox is an instrument) - pedalboard is the cart that wants it. docs/design/auv3-plugin-types.md §3.1 / §4.1",
    "SHIP IDENTITY: this app currently ships NO PLUG-IN, and the AU's name is hardcoded (OPEN, 2026-08-15). The AU extension is opt-in via the manifest's `auCart` key and apps/tinyacidjam/app.json does not set one, so testflight.sh STRIPS the AU target from the store spec: Tiny Acid Jam goes to the App Store as a standalone app with no AUv3 at all. One manifest line is the whole gap. And when it is set, the plug-in still misnames itself: testflight.sh patches the app's bundle id, version and CFBundleDisplayName from the manifest, but its own header says `the AU keeps its own hand-authored Info.plist`, so project.yml:121 CFBundleDisplayName `Tinyjam Demo`, project.yml:135 component name `Tinyjam: Demo` (the string a DAW SHOWS IN ITS PLUG-IN LIST) and the manufacturer/subtype codes (the auval triple) are the same for every app we build. device.sh patches nothing at all, so a cable install is always `TinyjamHello`. The icon is the one part already right, staged by build-app.js --ios from the manifest, provided you go through APP=<name> rather than a bare single-cart play.js staging (which stages ios/default-icon.png). Fix shape: derive the AU identity from the manifest the way the app identity already is (a few more sed lines in testflight.sh + manifest keys for the AU name and the four-char subtype) and give device.sh the display-name override. docs/design/auv3-plugin-types.md §6",
    "HOST PARAMETERS (a DAW can automate + record the knobs): SHIPPED (2026-08-15), except host READ-BACK. Came out of the maker asking 'is any of this recordable or usable in a way? not really I guess' - and the answer was mostly YES (the host records the MIDI notes, which is arrangement, the thing this rack never had) with one big NO: the AUv3 exposed NO AUParameterTree AT ALL, so a host saw ZERO parameters. Nothing automatable, nothing recordable, an empty lane menu. That also reframes the mod wheel work of the day before: mapping CC1 to the master filter was a WORKAROUND for having no parameters, not a design anyone would choose. THE MODEL - the parameter IS the knob: param_bind(addr, &knob, name, lo, hi) takes a pointer to a float the cart ALREADY OWNS, so the panel keeps dragging it, the DSP keeps reading it, and the host now reads and writes that same float. One storage location, not two, which is why 'the host automates it' / 'the panel follows automation' / 'a drag shows up in the host lane' are the SAME FACT rather than three sync paths to keep honest. The cart changed nothing else - 21 param_bind calls and no edit to any drawing or DSP code. The TREE IS BUILT FROM WHAT THE CART DECLARED, never a table in Swift: that would be a second source of truth for a fantasy console whose whole point is swapping carts. WHAT IS EXPOSED and why not everything: 21 - the MST performance set (FLT/RES/GLU/FB/PUMP), the acid five on each 303, DIST/SEND/VERB on each drum machine. The rack has ~40 knob sites; the per-voice drum tone knobs, the p-lock lanes and the pattern editors are deliberately OUT - they are how you BUILD the sound, the session blob already carries them, and every parameter lives in the host's menu forever. ADDRESSES ARE FOREVER: a saved project's automation stores nothing but the number, so changing what one means silently re-points somebody's lane at a different knob, with no version to refuse it and no error to see. Append, never renumber; retire one by leaving the hole (the gaps between groups are for growth). TWO TRAPS, both paid for: (1) the drain MUST live in loop_step and NOT de_frame - de_frame is the HOST's entry and the native loop calls loop_step directly, so a drain in de_frame works perfectly under a DAW and does NOTHING under play.js, which is to say nothing any gate in this repo can see; it cost one confused round with the trace showing a knob that would not move while the queue filled up behind it. (2) BIND IN init(), NOT draw(), and before the rolling AUTOSAVE RESTORE - the knobs are drawn only on the FOCUSED face so binding where they are drawn makes parameters appear and vanish from the host menu as you tap around, and param_bind captures the slot as the DEFAULT, so binding after the restore tells the host that last session's values are the factory ones and its 'reset parameter' returns you to Tuesday. GATED TWICE: tools/param-check/run.sh (9 assertions, the ENGINE half - table, queue, frame drain, range clamp, and that a write reaches the DSP; two negative controls, incl. a write to an addr NOBODY BOUND which must change nothing) and ./au-transport-check --params in ios/mac.sh, the only place the whole chain is real (21 parameters, named, addressed, and a host write closing the master filter 0.711 -> 0.281 with a no-op control first). New harness flag --param <addr>@<frame>=<value> goes through de_param_set, the same queue a DAW automation lane writes into. OPEN, and it is a real defect: HOST READ-BACK. Out of process a host reads back the value a parameter held BEFORE its own write. Measured, not guessed: the provider IS wired and IS reading real cart state (a parameter nothing wrote reads its exact cart default, 0.55), the write DOES reach the DSP, and it is NOT our push-back logic - disabling the panel-move poll entirely changed nothing, and neither did removing the suppression that stops a host write echoing back; both tested, both negative. Remaining suspect is out-of-process parameter MIRRORING (the host side caches the tree and may not consult the extension on every read). Consequence: a host's generic view and a reopened project can show a knob at its old value until something writes it again - automation WORKS, it may just not display where it starts. The gate reports it as a warning, not a failure, deliberately. ALSO OPEN: no ramping (the tree does not claim flag_CanRamp because the engine applies a value once per FRAME, so a fast sweep steps at 60Hz - telling the truth beats advertising smoothing we do not do), no factoryPresets, and no lint yet on the append-only address rule. Design: docs/design/host-parameters.md",
    ...
    "DENS knob - how MANY pauses, the sibling of GAP's how DEEP (SHIPPED 2026-07-28, the maker: 'i think we want a second knob for this'). The VOWEL page now carries FIVE knobs while SPK is lit: GLID/DENS/GAP/Q/MIX. Five is safe on this glass even though four was the previous high-water mark, and the reason is worth keeping: lcdknob_cell sizes r = min(h*0.30, w*0.42), and on this row HEIGHT is the binding term (8.1 vs 9.6 at five across), so the knob does NOT shrink going 4->5 - only its horizontal padding does. Grabbability is therefore unchanged, which is the thing that bit the roomy sweep panel once. THE MECHANISM, which avoids going random: the word is now 8 plain vowel slots plus a parallel VOW_RESTAT[] threshold array, and a slot becomes a rest when DENS reaches its threshold. So raising DENS adds pauses in a fixed, hand-chosen, MUSICAL order - deterministic and repeatable at every setting - where a per-note random rest chance would trade the word for a stutter. Thresholds are 9.0 on the strong syllables so they can never drop out (slots 0,2,4,6 are unreachable by a 0..1 knob). Rests arrive phrase-END first, then the mid-phrase breath, then the finer ones. SWEPT (500 frames each, isolated save-dir): DENS 0.0 -> rests {} 0% paused std 1275 Hz; 0.25 -> {7} 12% std 1740; 0.5 -> {3,7} 23% std 1964; 0.7 -> {3,5,7} 36% std 2267; 0.95 -> {1,3,5,7} 48% std 2457. Monotonic, and the placement order is exactly as authored. THE IMPORTANT GATE: DENS 0.5 (the default) rests slots 3 and 7, which is precisely the committed two-breath-group word - and the render is BYTE-IDENTICAL to the pre-DENS version (both sha e1b1dc2c3c6e56a7 on the same script/frames). Adding the knob did not move the sound, which is the property you want when a fixed structure becomes parametric; check it that way for any future knob carved out of a constant. DENS grabbability verified through the UI too: a scripted drag up added rests 5 and 1 to the resting set. OPEN: (1) the two axes are now orthogonal and both ear-check items - DENS sets the phrase rhythm, GAP its severity, and the musical question is which COMBINATION reads best (DENS 0.25 + GAP 1.0, one hard stop per phrase, is a different instrument from DENS 0.7 + GAP 0.5, a constant gentle pulsing). (2) The rest currently HOLDS the vowel and only gates; closing toward OO instead would read as a hummed m/w rather than silence. (3) VOW_RESTAT[] placement is hand-authored for THIS word - if the word ever becomes editable, the thresholds have to travel with it.",
    "GAP knob - pausiness is now a control (SHIPPED 2026-07-28, the maker: 'maybe the pausiness can be a knob in the screen? 4 will just fit?'). It does fit: the phone VOWEL page carries FOUR knobs while SPK is lit (GLID/GAP/Q/MIX), which the SWP page had already proven for this glass (FMIX/FFB/PMIX/PFB). Knob COUNT follows the mode - 3 hand-swept, 4 speaking - so GAP only exists when it means something and there is never a dead knob parked on the page. GAP IS DEPTH, NOT DENSITY: 0 = a rest does not close at all, 1 = the voice stops dead, middle = a DIP where it ducks without stopping. Depth was chosen over rest-density because every position on the axis is musical and the deterministic two-breath-group phrase survives, where sprinkling random rests would trade the word for a stutter. MEASURED sweep (500 frames, isolated save-dir, centroid std over 12 windows/sec): GAP 0.0 -> gate floor 1.000, 0% paused frames, std 1290 Hz; GAP 0.5 -> floor 0.500, std 1597 Hz; GAP 1.0 -> floor 0.000, 23% paused, std 1964 Hz. Cleanly monotonic, and note GAP=0 reproduces the pre-pause slur (1290 vs the 1270 measured then) so the knob is a genuine A/B of the whole idea from inside the instrument. Grabbability verified THROUGH THE UI, not assumed - the roomy sweep panel once shipped a ~13px knob that was effectively undraggable, so a 4-across row deserved the check: a scripted drag down the GAP knob raised the gate floor 0.000 -> 0.638. Also fixed here, found while touching the roomy panel: its MIX knob still had lay_grid(3,3,2) hardcoded AND a stale 0.9 reset-default from before vowmix moved to 0.7 - a knob's default arg is what DOUBLE-TAP resets to, so a stale one silently resets to the wrong value. All five knob defaults are now cross-checked against their declarations. OPEN: rest DENSITY (how many rests, not how deep) is still unexposed and is the natural sibling if the phrase rhythm itself wants tuning; the word is a const array, so it would want either a second knob or a small set of word presets.",
    "SPEAK gets PAUSES - the fix that turned a slur into speech (SHIPPED 2026-07-28, the maker's call: 'when speaking sometimes we need a little pause'). He was right and it is MEASURABLE. The word now carries explicit REST slots (VOW_REST, an 8-slot word = OO AH EE _ AH EH OH _, two breath groups): on a rest the voice LETS GO - the wet mix glides to 0 via a vowgate, the mouth closes, and that note comes through dry, which breathes the phrase AND lets the raw acid punch through. A rest lasts exactly one note, so the pause scales with tempo for free. THE PROOF, spectral centroid over time (12 windows/sec): static vowel = mean 2201 Hz std 1281 Hz; word with NO pauses = mean 2287 Hz std 1270 Hz; word WITH pauses = mean 2799 Hz std 1964 Hz. Read the middle row: a GAPLESS vowel-per-note word moves the spectrum NO MORE THAN A STATIC VOWEL DOES (1270 vs 1281). That is the honest negative from the previous session's SPEAK entry, now explained - it slurred, it never spoke, because the phrase had no boundary. Pauses take variance +55%. GENERAL LESSON for any per-event modulation: the gaps carry the phrasing, not the values. TWO BUGS FOUND ON THE WAY, both invisible by ear and both caught by the DE_TRACE watches: (1) DOUBLE-ADVANCE - both 303 lines feed one word, so notes landing on the same tick advanced it TWICE in a frame and SKIPPED slots; a skipped REST is a pause that never happens (traced: vstep walking 0,2,4,6 and only 6% of frames paused instead of 2-in-8). Fixed with vowfired: simultaneous notes are ONE syllable, because a mouth says one at a time. (2) TOO-SLOW CLOSE - a pause lasts one note (~6.8 frames for a 16th at 132 BPM) but easing shut on the GLID slew needed ~7 frames to even reach 0.25, so the mouth never finished closing and the 'pause' was an inaudible dip. Fixed by making the gate ASYMMETRIC (snap closed 0.55/frame, ease back open on GLID) - which is what a stop consonant physically is. Verified after both fixes: vstep walks 0..7 with ZERO skips over 66 advances, vgate reaches 0.000, 23% of frames sit below 0.25 (matching 2-in-8), and one pause traces 0.98 -> 0.44 -> 0.20 -> 0.09 -> 0.04 -> 0.02 -> 0.01 -> 0.19 -> 0.33 -> 0.45 -> 0.63. Both regressions are now watched by tools/clips/acidcandy/06-speak-vowel-per-note.script (if vstep skips, bug 1 is back; if the paused-frame share falls to single digits, bug 2 is). OPEN: (1) EAR CHECK still the real verdict - is TWO rests in eight the right amount of breath, and do they land in musical places? A GAP knob (rest density) is the obvious control if it wants tuning, but the page's three knob slots are full. (2) the rest currently HOLDS the vowel and only gates; closing toward OO instead would read as a hummed 'm/w' rather than silence - a different, softer pause worth trying. (3) everything in the previous SPEAK entry's OPEN list still stands, especially the instrument_formant-on-the-303s-only variant.",
    ...
    "waveshape flip: SHIPPED (2026-07-16) a SAW/SQR WAVE toggle in the 5th knob slot of the 303 DEEP page (flips a.wave + re-calls acid_define). DRVMODE: SHIPPED (2026-07-23) — a 4-way SOFT/HARD/FOLD/ASYM cycle (flips a.drvmode + re-calls acid_define), reachable in BOTH voicings. PHONE: in the FX SCREEN, a lcdbtn in the bottom toggle strip directly under the DRV knob (paired with the CLEAN/RAW SAW toggle — the two waveshaping switches together); the DRV *amount* stays the knob above it. (An earlier 2×2 corner-switch-bank attempt was dropped — the split slot was ~8px and labels collided; the maker's call was to move it into the screen next to DRV.) ROOMY (iPad): the same cycle sits RIGHT OF THE ACC KNOB, one per 303, a cbtn with a DRV caption below it mirroring the ACC knob's label. Both views write the same a->drvmode.",
...
// (the wet mix glides to 0, the mouth closes) and that note comes through dry, which both breathes
// the phrase and lets the raw acid punch through. A rest lasts exactly one note, so the pause scales
// with tempo for free. Glided by the same GLID slew, so a pause opens and closes, never clicks.
// The word is EIGHT vowel slots against a 16-step pattern that only advances on ON steps, so the
// phrase lands differently bar to bar (the polymeter drift this cart already likes in its per-line
// loop lengths). Built from pedalboard's six vowels (its indices 0,2,4,2,3,1 over 4) plus two more.
static const float VOW_WORD[8] = { 0.0f, 0.5f, 1.0f, 0.75f, 0.5f, 0.75f, 0.25f, 0.0f };
#define VOW_NWORD 8
// ...and WHICH slots become rests is a continuous knob, without going random. Each slot carries the
// DENS value at which it turns into a rest; a slot rests when DENS reaches its threshold. So raising
// DENS adds pauses in a fixed, hand-chosen, musical ORDER — deterministic and repeatable at any
// setting, where a random per-note rest chance would trade the word for a stutter. Thresholds > 1
// are the strong syllables that never drop out. Rests appear phrase-END first (slot 7), then the
...
static void  vow_attack(void) {              // a fresh 303 note landed (NOT a tie — a tie holds, so it is no new syllable)
    if (!vowspeak || vowfired) return;        // already spoke this frame: both lines on one tick = ONE syllable
    vowfired = 1;
    vowstep = (vowstep + 1) % VOW_NWORD;
    if (vowdens >= VOW_RESTAT[vowstep]) vowgtgt = 1.0f - vowgap;   // a REST: hold the vowel, close the voice by GAP
    else { vowgtgt = 1.0f; vowtgt = VOW_WORD[vowstep]; }           // a syllable: open up and glide to its vowel
}
static void  vow_tick(void) {                // every frame: ease the vowel + the voice gate toward target
    vowfired = 0;                            // new frame → the mouth is free to say the next syllable
    if (!vowspeak) { vowcur = vowtgt = vowvow; vowgate = vowgtgt = 1.0f; return; }   // parked on the knob while off, so arming SPEAK starts from where you left it
    float k = 0.04f + vowglide * 0.40f;                        // GLID 0..1 = slurred/slow ... crisp/snappy
    vowcur += (vowtgt  - vowcur) * k;
    // The gate is ASYMMETRIC, and it has to be: a pause lasts one note (~6.8 frames for a 16th at
    // 132 BPM), but easing shut on the GLID slew needs ~7 frames to even reach 0.25 — so the mouth
    // never finished closing and the "pause" was an inaudible dip (measured: 4% of frames, and the
    // pause is 2-in-8 slots). A real mouth SHUTS fast (that is what a stop consonant is) and opens
    // smoothly, so: snap closed, ease back in on GLID. Now the gap is a gap.
    vowgate += (vowgtgt - vowgate) * (vowgtgt < vowgate ? 0.55f : k);
}
...
// ── THE HOST'S KEYBED → the FOCUSED machine ──────────────────────────────────────────────────────
// docs/design/host-midi-notes.md. Host notes ALREADY reached the engine ring (the AUv3 parses them
// at ios/AU/TinyjamAU.swift and calls de_midi_event); nothing drained them, so GarageBand drew a
// keyboard above the plug-in that did nothing. The plumbing was never the work — what a note MEANS
// on a rack of five machines is, and this is the doc's TOP ROW: **the PATTERN is the instrument.**
// On a drum face a note plays the KIT; anywhere else it TRANSPOSES the acid lines. There is no mode
// switch, deliberately: the other row (play one VOICE chromatically) is not built, and a switch
// between one thing and nothing is just a thing to get wrong.
//
// The focused FACE decides which of the two you get. Phone view shows one face; the roomy rack
// draws all five at once, so there the focus is the panel you last tapped.
...
#define HN_MAX 8
static void hn_push(int note) {
    for (int i = 0; i < hn_n; i++) if (hn_stack[i] == note) return;   // already held — a re-trigger, not a second key
    if (hn_n < HN_MAX) hn_stack[hn_n++] = note;
}
static void hn_pop(int note) {
    for (int i = 0; i < hn_n; i++) if (hn_stack[i] == note) {
        for (int j = i; j < hn_n - 1; j++) hn_stack[j] = hn_stack[j + 1];
        hn_n--; return;
    }
}

// ── TRANSPOSE ──
// A held key is the acid lines' ROOT. Both lines move together: they are a bass and lead duo an
// octave apart, and transposing one of a pair leaves your song half in the wrong key.
//
// It is engaged only on a NON-drum face, so finger-drumming the kit cannot silently re-key the 303s
// underneath you — the face you are looking at is the face the keybed talks to, with no exceptions
// to remember.
//
// ⚠ It OVERRIDES rather than WRITES, exactly like the mod wheel: `mroot` keeps whatever the player
// dialled in, so letting every key up hands the KEY panel straight back — and a MIDI take can never
// dirty the SAVED song, which matters because `mroot` lives in CartState and a project remembers it.
//
...
// ── the KIT ──
// note → voice, the General MIDI map read BACKWARDS. The SAME table the rack already sends on
// (MO_GM808/MO_GM909, defined with the MIDI-out block below), so our input and our output agree by
// construction and a pad controller lands on the voices its own labels promise. That is not luck:
// GM's percussion map was modelled on these two machines. -1 = this note is not in the kit.
static int gm_voice(int m, int note);   // defined after MO_GM808/909, which need the TR_* rosters

// A host note on a drum face does what a finger on that voice's pad does — select it, fire it, and
// with REC lit while playing PUNCH it onto the current step. That last one is live MIDI step-record,
// and it cost nothing: the REC grammar was already there for fingers.
//
// ONE DELIBERATE DIVERGENCE from the pad: this always SOUNDS. A pad tap is a select that sometimes
// sounds (silent while the transport runs, because an off-grid hit would smear the groove and REC is
// the audible opt-in). A MIDI note is never a select — playing is the only thing it is for — so
// gating it on REC would make the keybed dead exactly when someone is jamming with it.
static void hn_drum(int m, int note, int vel) {
    int v = gm_voice(m, note); if (v < 0) return;
    int bo = (vel >= 100) ? 2 : 1;   // hard = the pattern's own accent (+2); normal = a pad tap's 1
    if (m == M_808) {
        dsel = v; dtrig[v] = 1;
        if (drec_now && playing) dgrid[v][s_step] = 1;
        tr808_fire(TR808_BASE, v, bo, 0, dtune, ddecay, dcolor);
    } else {
        d9sel = v; d9trig[v] = 1;
        if (drec_now && playing) d9grid[v][s_step] = 1;
        tr909_fire(D909_BASE, v, bo, 0, d9tune, d9decay, d9color);
    }
}
...
static void apply_fx(void) {
    ...
    // master SWEEP — TWO INDEPENDENT whole-mix devices for breakdown risers: a FLANGER (metallic jet)
    // and a PHASER (hollow swirl), each on/off on its own so you can run 2 / 1 / 0 at once (they stack).
...
static void navspine(Box nav) {
    ...
    {   // PLAY / STOP
        ...
        // The tap is refused only when the clock DRIVES the transport — then toggling `playing`
        // would flicker for the one frame before update() overwrites it, which reads as a bug.
        // On a TEMPO-ONLY clock the button stays live, because otherwise the rack has no way to
        // start at all (that shipped once; see sync_transport in runtime/sync.h).
        int xport = sync_transport();
        // ⚠ WHILE SLAVED THIS REGISTERS NO WIDGET AT ALL — the same rule the TEMPO and FLT knobs
        // already follow (see host_knob_ext). It used to call ui_button_core unconditionally, so
        // under a host it drew the full press feedback (focus ring, white `hot` outline) for a tap
        // it was then going to refuse. That is the transport twin of the silent-rotation bug: the
        // control ANSWERS your finger and does nothing, which reads as broken rather than as
        // surrendered. The maker hit it in GarageBand on 2026-08-15 — "the play button gets a white
        // outline when pressing it down but doesn't change from play to pause".
        // It is a LAMP under a host, not a button: same geometry so nothing jumps when the host
        // connects, green = their play, grey = their stop.
        if (!xport) {
        ...
        rrectfill(px, py, pw, ph, 2, xport ? (playing ? CLR_DARK_GREEN : CLR_DARKER_GREY)   // slaved: green = their play, grey = their stop
                                           : (playing ? CLR_TRUE_BLUE : CLR_DARK_BROWN));
        rrect(px, py, pw, ph, 2, hot ? CLR_WHITE : CLR_BROWNISH_BLACK);   // hot is always 0 when slaved
...
static void draw_303(Box stage, int i) {
    ...
    if (pscreen[i] == PS_FLAG) {
        ...
        // the bottom strip holds the two waveshaping toggles: DRV MODE cycle (left, under the DRV knob) +
        // the SAW band-limit. MODE cycles the drive FLAVOUR (SOFT tanh / HARD clip / FOLD wavefolder / ASYM
        // tube) — the DRV *amount* stays the knob above it. Flips a->drvmode + re-defines the voice.
        Box mode = lay_split(clean, EDGE_LEFT, clean.w * 0.34f, &clean);
        static const char *DM[4] = { "SOFT", "HARD", "FOLD", "ASYM" };
        if (lcdbtn(0x47u, (int)mode.x, (int)mode.y, (int)mode.w - 2, (int)mode.h, DM[a->drvmode & 3], a->drvmode != 0)) { a->drvmode = (a->drvmode + 1) & 3; acid_define(a); }
        ...
        // ⚠ SURRENDERED while the host holds a key: no widget is registered at all, so a finger
        // cannot rotate a control something else is driving (the rule the TEMPO knob, the FLT knob
        // and the transport button each learned the hard way), and the host's key is drawn WHITE
        // against a dimmed strip so "not yours right now" reads without a word. There is no room
        // for one on a 160×100 face; the roomy keyboard gets the word (see r2_keyboard).
        int hostkey = host_root_on();
        for (int k = 0; k < 12; k++) {
            ...
            if (!hostkey && ui_button_core(w, px, py, pw, ph, &foc, &pr, &hot)) mroot[i] = k;
            int lit = (root_live(i) == k);
            int fill = lit ? (hostkey ? CLR_WHITE : CLR_LIME_GREEN)
                           : sharp ? CLR_BROWNISH_BLACK : hostkey ? CLR_BLUE_GREEN : CLR_DARK_GREEN;
            rrectfill(px, py, pw, ph, 1, fill);
            rrect(px, py, pw, ph, 1, lit && hostkey ? CLR_LIME_GREEN : (lit || hot) ? CLR_WHITE : CLR_DARK_GREEN);
    ...
    if (seq_grid && keys_mode) {
        ...
        for (int k = 0; k < 16; k++) {
            ...
            if (c && ui_grabbed(w)) {                                        // fire once on grab (tap)
                if (k <= 12) {                                               // note → write sel + advance
                    ...
                    if (playing) ac_note(&ac[i], ac[i].base + root_live(i) + loct[i] * 12 + k + key_oct * 12, acc[i][s_sel[i]], 0);
        ...
        for (int n = 0; n < 12; n++) if (!isblack[n]) {
            ...
            if (ui_button_core(wid, x, ky, 20, kh, &foc, &pr, &hot)) { pit[i][s_sel[i]] = n; s_on[i][s_sel[i]] = 1; mbop = 1; ac_note(a, a->base + root_live(i) + loct[i] * 12 + n, 0, 0); }
        ...
        for (int n = 0; n < 12; n++) {
            if (isblack[n]) {
                ...
                if (ui_button_core(wid, x, ky, 12, 9, &foc, &pr, &hot)) { pit[i][s_sel[i]] = n; s_on[i][s_sel[i]] = 1; mbop = 1; ac_note(a, a->base + root_live(i) + loct[i] * 12 + n, 0, 0); }
                int lit = pit[i][s_sel[i]] == n && s_on[i][s_sel[i]];
    ...
    if (mstflow == 0) {
        ...
        if (fxpage < 0) {
            ...
            { const char *nm = fx_name(fxpage);
              ...
              // SPK ARMS THE DEVICE. It used to be an independent toggle, and because SPK sits LEFT of
              // ON, tapping it first is the natural move — which gave a lit SPK, pips walking, and
              // DEAD SILENCE (proven byte-identical to fully dry: with vowon 0 the formant mix is 0, so
              // SPEAK is inert no matter how correctly it runs). That is the third time this feature
              // presented as "it does nothing" for a reason that was not the feature. Turning ON
              // "speak" obviously means you want to hear it, so it arms; ON stays the master kill.
              if (fxpage == FX_FORMANT) { Box sb = lay_split(hdr, EDGE_RIGHT, lay_clamp(hdr.w * 0.34f, 16, 28), &hdr);
                  if (lcdbtn(0x30Bu, (int)sb.x, (int)sb.y, (int)sb.w - 2, (int)sb.h, "SPK", vowspeak)) {
                      vowspeak = !vowspeak; if (vowspeak) vowon = 1; } }
              font(FONT_TINY); print(nm, (int)hdr.x + 2, (int)hdr.y + 1, CLR_LIME_GREEN);
              // ── the WORD readout ── 8 pips: the live syllable lit, rest slots drawn as a low tick.
              // This exists because SPEAK was UNREADABLE without it. The word only advances on a 303
              // note-on, so with both 303s muted (which is how the rack BOOTS) nothing triggers, the
              // vowel freezes wherever it sat, and SPK looks like it "just repeats the current vowel" —
              // with no way to tell, since VOWL has been replaced by GLID so you cannot even sweep it
              // by hand. Rule 6 of control-vocabulary.md, self-inflicted: a state you cannot SEE is not
              // a control. A frozen pip row now says "nothing is feeding me" at a glance, and a walking
              // one says the mechanism is fine so any remaining problem is voicing, not wiring. It also
              // makes DENS legible — you watch rests appear in the word as you turn it.
              if (fxpage == FX_FORMANT && vowspeak) {
                  int px = (int)hdr.x + text_width(nm) + 5, py = (int)hdr.y + 2;
                  for (int s = 0; s < VOW_NWORD && px + s * 4 + 2 < (int)(hdr.x + hdr.w); s++) {
                      int rest = (vowdens >= VOW_RESTAT[s]), live = (s == vowstep);
                      // DIM the whole row when the device is disarmed: a row that walks while nothing
                      // is routed is a readout that LIES, which is worse than no readout.
                      int col  = !vowon ? CLR_DARK_GREEN
                               : live   ? CLR_WHITE : rest ? CLR_DARK_GREEN : CLR_MEDIUM_GREEN;
                      if (rest && !live) line(px + s * 4, py + 2, px + s * 4 + 2, py + 2, col);   // a rest = a low tick, not a block
                      else               rectfill(px + s * 4, py, 3, 3, col);
                  }
              } }
            if (fxpage == FX_FORMANT) {
                ...
                // Knob COUNT follows the mode: 3 while hand-swept, 5 while speaking. Five is safe on
                // this glass even though 4 was the previous high-water mark, because lcdknob_cell sizes
                // r = min(h*0.30, w*0.42) and HEIGHT is the binding term here (8.1 vs 9.6 at five
                // across) — so the knob does NOT shrink going 4→5, only its horizontal padding does.
                // The speech knobs only exist while SPK is lit, so no knob ever sits there dead.
                int nk = vowspeak ? 5 : 3;
                if (vowspeak) { lcdknob_cell(lay_grid(gc, nk, nk, 0, 2), &vowglide, "GLID", 0.35f);
                                lcdknob_cell(lay_grid(gc, nk, nk, 1, 2), &vowdens,  "DENS", 0.5f);    // how MANY pauses
                                lcdknob_cell(lay_grid(gc, nk, nk, 2, 2), &vowgap,   "GAP",  1.0f); }  // how DEEP each one
                else            lcdknob_cell(lay_grid(gc, nk, nk, 0, 2), &vowvow,   "VOWL", 0.5f);
                lcdknob_cell(lay_grid(gc, nk, nk, nk - 2, 2), &vowq,   "Q",    0.6f);
                lcdknob_cell(lay_grid(gc, nk, nk, nk - 1, 2), &vowmix, "MIX",  0.7f);
            } else {
...
//
// ▶ SEAM — THIS IS ALREADY A COMPLETE SEQUENCER, AND A DAW CANNOT HEAR IT.
// Everything below (slides as overlapping notes, accents as velocity, real drum gate lengths, the
// 24ppqn clock, start/stop, the shutdown flush) is finished work that goes to a CoreMIDI VIRTUAL
// SOURCE. That is the STANDALONE path. A plug-in's MIDI reaches its host through the AU's
// MIDIOutputEventBlock, which the engine has no seam for at all (ios/Sources/engine.h exports
// audio-in, notes, bend, CC and transport, and nothing outward), so under a DAW this output goes
// out a side door the project cannot record. THE CART SIDE IS DONE: `mout_on` already gates it and
// the pattern already streams. What is missing is engine + Swift, and once it lands this rack is
// also an `aumi` MIDI processor that plays somebody else's instruments.
// Survey + the proposed de_midi_out_drain shape: docs/design/auv3-plugin-types.md §3.2 / §4.2.
//
...
// …and the SAME tables read backwards, for the host's keybed (host_notes, up top). Declared there,
// defined here because it needs the rosters. One map, both directions: what we send on a note is
// what we answer to, so a DAW round-trip and a pad controller agree without a second table to drift.
static int gm_voice(int m, int note) {
    if (m == M_808) { for (int v = 0; v < TR_NV;  v++) if (MO_GM808[v] == note) return v; }
    else            { for (int v = 0; v < TR9_NV; v++) if (MO_GM909[v] == note) return v; }
    return -1;
}
...
// docs/design/host-parameters.md. Before this the AU exposed NO parameter tree at all, so a host saw
// zero parameters: nothing automatable, nothing recordable, the lane menu empty. (It is also why the
// mod wheel was mapped to the master filter — a workaround for having none, not a free choice.)
//
// WHAT IS EXPOSED, and why not everything: the rack has ~40 knob sites and every parameter shows up
// in the host's automation menu forever, so this is the PERFORMANCE set — the controls you would
// actually draw a lane for. The per-voice drum tone knobs, the p-lock lanes and the pattern editors
// are deliberately out: they are how you BUILD the sound, and the session blob already carries them.
//
// ⚠ THE ADDRESSES ARE FOREVER. A saved project's automation stores nothing but this number, so
// changing what one means silently re-points somebody's lane at a different knob — no version to
// refuse it, no error to see. APPEND, NEVER RENUMBER; retire one by leaving its hole. The gaps
// between groups are there so a group can grow without disturbing the next.
enum {
    P_M_FLT = 1, P_M_RES, P_M_GLU, P_M_FB, P_M_PUMP,      // MST: the whole-rack performance controls
    P_A_CUT = 10, P_A_RES, P_A_ENV, P_A_DEC, P_A_ACC,     // 303a — the acid five
    P_B_CUT = 20, P_B_RES, P_B_ENV, P_B_DEC, P_B_ACC,     // 303b
    P_8_DIST = 30, P_8_SEND, P_8_VERB,                    // 808 machine FX
    P_9_DIST = 40, P_9_SEND, P_9_VERB,                    // 909 machine FX
};

// Bound in init(), NOT in draw(). Two reasons, and the second is the sharp one: the knobs are drawn
// only on the FOCUSED face, so binding where they are drawn would make parameters appear and vanish
// from the host's menu as you tap around. And param_bind captures the slot's current value as the
// DEFAULT — from init() that is the factory value, whereas from the first frame a session restore
// has already landed and the host would be told the restored value is the default.
static void bind_params(void) {
    param_bind(P_M_FLT,  &mflt,  "FLT",  0, 1);   // the DJ filter — the one gesture that moves all five machines
    param_bind(P_M_RES,  &mfres, "RES",  0, 1);
    param_bind(P_M_GLU,  &mglu,  "GLU",  0, 1);
    param_bind(P_M_FB,   &mfb,   "FB",   0, 1);
    param_bind(P_M_PUMP, &mpump, "PUMP", 0, 1);
    static const int AK[5] = { ACID_CUT, ACID_RES, ACID_ENV, ACID_DEC, ACID_ACC };
    static const char *AN[5] = { "CUT", "RES", "ENV", "DEC", "ACC" };
    for (int k = 0; k < 5; k++) {
        param_bind(P_A_CUT + k, &ac[0].p[AK[k]], AN[k], 0, 1);
        param_bind(P_B_CUT + k, &ac[1].p[AK[k]], AN[k], 0, 1);
    }
    param_bind(P_8_DIST, &dist8,     "DIST", 0, 1);
    param_bind(P_8_SEND, &msend[2],  "SEND", 0, 1);
    param_bind(P_8_VERB, &fxverb[2], "VERB", 0, 1);
    param_bind(P_9_DIST, &dist9,     "DIST", 0, 1);
    param_bind(P_9_SEND, &msend[3],  "SEND", 0, 1);
    param_bind(P_9_VERB, &fxverb[3], "VERB", 0, 1);
}
...
void init(void) {
    ...
    bind_params();   // ⚠ BEFORE the autosave restore below, which rewrites these very knobs. Bind
                     // after it and the host is told your LAST SESSION's values are the factory
                     // defaults, so its "reset parameter" would return you to Tuesday.
...
void update(void) {
    // EXTERNAL CLOCK (runtime/sync.h) — a DAW's MIDI sync, a drum machine's clock out, later an
    // AUv3 host or Ableton Link. While one is feeding us it owns all three of tempo, transport
    // and POSITION, so the TEMPO knob and the transport button go read-only rather than fight it
    // (the ReBirth-on-iPad model: be a proper slave, don't half-follow). Unplug and we keep
    // playing at the tempo we last heard, because sync.h hands control back after a 2s silence.
    bool ext = sync_active();
    if (ext) {
        if (sync_bpm() > 20) g_bpm = sync_bpm();
        // TRANSPORT only if the clock actually drives one. A bare MIDI clock may never send
        // START/STOP, and taking its transport anyway pinned the rack to stopped with the play
        // button already surrendered = UNSTARTABLE (the maker's first real Live session). On a
        // tempo-only clock we borrow the tempo and keep our own play/stop.
        if (sync_transport()) {
            bool xplay = sync_playing();
            if (xplay && !playing) {                                   // their PLAY = our transport button
                laststep = -1; laststep303[0] = laststep303[1] = -1;
                for (int m = 0; m < M_N; m++) armpat[m] = -1;
            }
            playing = xplay;
        }
    ...
    g_last_t = t;                                                      // how FAST, never WHERE, so it can't
                                                                       // follow a re-start or a loop jump
    ...
    if (playing) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (lcs != laststep303[i]) {
                ...
                if (s_on[i][ls]) {
                    int midi = ac[i].base + root_live(i) + loct[i] * 12 + pit[i][ls] + oct[i][ls] * 12 + oshift;
    ...
    // HOST KEYBED (docs/design/host-midi-notes.md, gated by tools/midi-note-check). hn = keys the
    // host is holding; hroot = the root it is imposing, or -1 when the panel still owns the key.
    // root0 is what the line ACTUALLY plays, which is the only one that proves the override reached
    ...
    // HOST PARAMETERS (docs/design/host-parameters.md, gated by tools/midi-note-check §9). npar = how
    // many the cart bound; pflt/pcut are two of them read back, which is what proves a host write
    // reached the KNOB rather than just the table.
    watch("npar", "%d", param_count()); watch("pflt", "%.3f", (double)mflt);
    watch("pcut", "%.3f", (double)ac[0].p[ACID_CUT]);
    ...
    watch("vgate", "%.3f", (double)vowgate);   // the voice gate: dips toward 0 on a word's REST slots (the pause)
#endif
}
...
// MST SWEEP panel — TWO independent whole-mix devices (breakdown risers): FLG (flanger, metallic jet)
// and PHS (phaser, hollow swirl), each on/off on its own so you can run 2/1/0 at once (they stack).
// FLG/PHS device toggles + shared tempo-synced rate (1/2/4-BAR) on TOP; below, EACH device owns its
// MIX + FB knobs (FLANGER: FMIX/FFB · PHASER: PMIX/PFB) so you can blend them independently. Big enough
// to grab (the tall panel gives the knobs room). The engine's own LFO does the swirl (see apply_fx).
static void r2_mstsweep(Box b) {
    Box m   = lay_inset(b, 2);
    Box top = lay_split(m, EDGE_TOP, m.h * 0.30f, &m);   // button row on top; m = the knob row below
    int tx  = (int)top.x;
    if (lcdbtnf(0x2D8u, &tx, (int)top.y, (int)top.h - 1, "FLG", flon)) flon = !flon;   // FLANGER device on/off
    if (lcdbtnf(0x2DCu, &tx, (int)top.y, (int)top.h - 1, "PHS", phon)) phon = !phon;   // PHASER device on/off
    static const char *RB[3] = { "1BAR", "2BAR", "4BAR" };
    for (int r = 0; r < 3; r++) if (lcdbtnf(0x2D9u + r, &tx, (int)top.y, (int)top.h - 1, RB[r], mswdiv == r)) mswdiv = r;
    lcdknob_cell(lay_grid(m, 4, 4, 0, 3), &flmix, "FMIX", 0.5f);   // FLANGER mix + feedback
    lcdknob_cell(lay_grid(m, 4, 4, 1, 3), &flfb,  "FFB",  0.5f);
    lcdknob_cell(lay_grid(m, 4, 4, 2, 3), &phmix, "PMIX", 0.5f);   // PHASER mix + feedback
...
static void r2_mstformant(Box b) {
    ...
    if (lcdbtnf(0x313u, &tx, (int)top.y, (int)top.h - 1, "SPEAK", vowspeak)) {                       // syllable-per-note
        vowspeak = !vowspeak; if (vowspeak) vowon = 1; }                                             // ...and it ARMS the device (see the phone page)
    if (vowspeak) {                                  // the WORD readout — see the phone page for WHY (a frozen row = nothing is feeding it)
        int py = (int)top.y + (int)top.h / 2 - 2;
        for (int s = 0; s < VOW_NWORD && tx + s * 5 + 3 < (int)(top.x + top.w); s++) {
            int rest = (vowdens >= VOW_RESTAT[s]), live = (s == vowstep);
            int col  = !vowon ? CLR_DARK_GREEN            // disarmed → dim, so the row can't walk while silent
                     : live   ? CLR_WHITE : rest ? CLR_DARK_GREEN : CLR_MEDIUM_GREEN;
            if (rest && !live) line(tx + s * 5, py + 3, tx + s * 5 + 3, py + 3, col);
            else               rectfill(tx + s * 5, py, 4, 4, col);
        }
    }
    int nk = vowspeak ? 5 : 3;                                                           // the speech knobs appear only while SPEAK is on
    if (vowspeak) { lcdknob_cell(lay_grid(m, nk, nk, 0, 3), &vowglide, "GLID", 0.35f);   // SPEAK owns the vowel → knob 1 becomes the slew
                    lcdknob_cell(lay_grid(m, nk, nk, 1, 3), &vowdens,  "DENS", 0.5f);    // how MANY rests the word takes
                    lcdknob_cell(lay_grid(m, nk, nk, 2, 3), &vowgap,   "GAP",  1.0f); }  // how DEEP each rest pauses
    else            lcdknob_cell(lay_grid(m, nk, nk, 0, 3), &vowvow,   "VOWL", 0.5f);
    lcdknob_cell(lay_grid(m, nk, nk, nk - 2, 3), &vowq,   "Q",    0.6f);
...
static void r2_keyboard(Box b, int i) {
    ...
    int hostkey = host_root_on();                     // the host holds the root → surrender (see the phone KEY strip)
    for (int k = 0; k < 7; k++) {                     // white keys (full height)
        int px = bx + k * ww, pw = ww - 1, lit = (root_live(i) == WMIDI[k]);
        int pr = 0, hot = 0, fo = 0; void *w = ui_wid_hash(0x250u + k, px, by, pw, bh);
        if (!hostkey && ui_button_core(w, px, by, pw, bh, &fo, &pr, &hot)) mroot[i] = WMIDI[k];
        rrectfill(px, by, pw, bh, 1, lit ? (hostkey ? CLR_WHITE : CLR_LIME_GREEN) : hostkey ? CLR_BLUE_GREEN : CLR_DARK_GREEN);
        rrect(px, by, pw, bh, 1, lit && hostkey ? CLR_LIME_GREEN : (lit || hot) ? CLR_WHITE : CLR_MEDIUM_GREEN);
    ...
    for (int j = 0; j < 5; j++) {
        int px = bx + BK[j][1] * ww - bkw / 2, lit = (root_live(i) == BK[j][0]);
        int pr = 0, hot = 0, fo = 0; void *w = ui_wid_hash(0x258u + j, px, by, bkw, bkh);
        if (!hostkey && ui_button_core(w, px, by, bkw, bkh, &fo, &pr, &hot)) mroot[i] = BK[j][0];
        rrectfill(px, by, bkw, bkh, 1, lit ? (hostkey ? CLR_WHITE : CLR_LIME_GREEN) : CLR_BROWNISH_BLACK);
        rrect(px, by, bkw, bkh, 1, lit && hostkey ? CLR_LIME_GREEN : (lit || hot) ? CLR_WHITE : CLR_BLACK);
    }
    // …and the WORD, which only fits here: the keyboard is width-capped at 7 keys, so whatever the
    // cell has left over is free space. Same grammar as the FLT knob's "MOD" and the tempo knob's
    // "EXT" — name the hand that is on the control.
    if (hostkey && bx + 7 * ww + 20 <= (int)(b.x + b.w)) {
        font(FONT_TINY); print("MIDI", bx + 7 * ww + 2, by + bh / 2 - 2, CLR_LIME_GREEN);
    }
}
...
static void r2_col303(Box c, int i) {
    ...
    { Box accr = lay_split(knob, EDGE_BOTTOM, knob.h / 3, &knob);   // ACC + DRV-mode share the CORE cluster's bottom row
      ...
      r2_kcell(lay_grid(accr, 2, 1, 0, 1), &a->p[ACID_ACC], "ACC", 0.55f, mac[m].col);   // ACC on the left
      // DRV waveshaper-MODE cycle, right of ACC (one per 303). Cycles the drive FLAVOUR — SOFT tanh /
      // HARD clip / FOLD sine wavefolder (the "mental" one) / ASYM even-harmonic tube; the DRV *amount*
      // stays the FX-row knob. Flips a->drvmode + re-defines the voice; reachable in BOTH voicings.
      { Box drvc = lay_grid(accr, 2, 1, 1, 1);
        static const char *DM[4] = { "SOFT", "HARD", "FOLD", "ASYM" };
        int dcw = (int)drvc.w, bw2 = (int)lay_clamp((float)(dcw - 6), 20, 40);
        int bx2 = (int)drvc.x + (dcw - bw2) / 2, by2 = (int)drvc.y + 2;
        if (cbtn(0x1C0u + i, bx2, by2, bw2, 11, DM[a->drvmode & 3], 0)) { a->drvmode = (a->drvmode + 1) & 3; acid_define(a); }
        font(FONT_TINY); print("DRV", bx2 + bw2 / 2 - 6, by2 + 13, R2_DIM); } }   // label BELOW the button, mirroring the ACC knob's label
    ...
    // the LIVE root, and in the host's colour while the host holds it — a readout that keeps showing
    // the panel's value while something else drives the sound is a readout that lies (the SPEAK rule).
    print(NOTE[root_live(i)], mx + 18, (int)keyR.y + 1, host_root_on() ? CLR_LIME_GREEN : mac[m].col);
