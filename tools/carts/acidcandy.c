/* de:meta
{
  ...
  "todo": [
    "SWEEP macro: SHIPPED on PHONE (2026-07-23) — a whole-mix phaser/flanger for breakdown risers, on a new MST 'SWP' LCD page (5th left soft-key, mstflow 6). Controls: a FLG/PHS flavour toggle (FLG = metallic jet, PHS = hollow swirl) + tempo-synced rate (1/2/4-BAR buttons) on top, SWEEP (mix/depth macro) + FB (feedback = the 'mental' intensity) knobs below. The engine's own LFO does the swirl (srate = g_bpm/(240*bars)); flanger()/phaser() are set-and-hold, reconfigured only on change in apply_fx (fx-frame lint clean), FLG+PHS mutually exclusive (idle one held dry). State: msweep/msweepfb/mswmode/mswdiv. Verified audible: sweep-on vs off WAV correlate 0.55 (same take, phaser-notched texture), no NaN. ROOMY: SHIPPED too (2026-07-23) — a SWP chip + r2_mstsweep panel on the MST big screen (r2_mstpanel 4), single wide row (buttons left, SWEEP+FB knobs right third — a 2-row stack collided the knobs into the buttons). OFF STATE (2026-07-23): the flavour toggle is now 3-state OFF/FLG/PHS (was FLG/PHS) — mswmode 0 = OFF forces both effects dry regardless of the knob, the reliable kill switch (the maker couldn't tell how to turn it back off: closing the SWP panel is just a view, and zeroing the knob wasn't obvious). Default mswmode = 0 so the rack boots dry. NOTE: acidcandy renders are NON-DETERMINISTIC (the 303 drift LFO_SHAPE_RANDOM wander, uncontrolled by --seed) → wav-correlate is USELESS for A/B here (two identical renders correlate ~0.5); verify sweep audibly by ear or a drift-robust metric (wav-modrate on the phaser LFO), NOT sample correlation. OPEN: (1) NOT in the SONG snapshot yet (deferred with the rest of SAVE); (3) play-feel tuning — default flavour (PHS), depth curve (0.55+0.45*mix), fb scale (*0.9), and whether it wants a per-303 phaser too are all ear-check candidates; (4) maybe a drawable SWEEP automation lane (draw the riser per-step) as the PCF/CRU/GAT sibling.",
...
static int   mswmode = 0, mswdiv = 0;                       // SWEEP flavour/on-off: 0 = OFF, 1 = FLG metallic jet, 2 = PHS hollow swirl. + rate div (0 = 1-bar, 1 = 2-bar, 2 = 4-bar sweep, tempo-synced)
...
static void apply_fx(void) {
    ...
    static float aSwp = -2, aSwpFb = -2, aSwpRate = -2; static int aSwpMode = -3;
    {
        ...
        if (msweep != aSwp || msweepfb != aSwpFb || mswmode != aSwpMode || srate != aSwpRate) {
            ...
            if (mswmode == 0 || msweep < 0.02f) { flanger(0.3f, 0, 0, 0);   phaser(0.4f, 0, 0, 0, 6); }   // OFF (flavour OFF or knob at 0) → both dry
            else if (mswmode == 1)              { phaser(0.4f, 0, 0, 0, 6); flanger(srate, dep, fb, mix); }   // FLG jet
...
static void draw_mst(Box stage) {
    ...
    if (mstflow == 0) {
        ...
        { static const char *SM[3] = { "OFF", "FLG", "PHS" };   // 3-state: OFF kills the sweep (regardless of the knob), FLG/PHS pick the flavour
          if (lcdbtn(0x70u, (int)mbtn.x, (int)mbtn.y, (int)mbtn.w - 2, (int)mbtn.h, SM[mswmode], mswmode != 0)) mswmode = (mswmode + 1) % 3; }
...
// MST SWEEP panel — the whole-mix phaser/flanger MACRO (breakdown riser), the roomy twin of the phone
// SWP page: FLG/PHS flavour + tempo-synced rate (1/2/4-BAR) on top, SWEEP (mix/depth) + FB (feedback =
// the "mental" intensity) knobs below. The engine's own LFO does the swirl (see apply_fx).
static void r2_mstsweep(Box b) {
    Box m = lay_inset(b, 2);
    // ONE wide row (the roomy panel is short + very wide): FLG/PHS + rate buttons flow on the LEFT,
    // the SWEEP + FB knobs sit in the RIGHT third. (A 2-row stack collided the knobs into the buttons.)
    Box kn = lay_split(m, EDGE_RIGHT, m.w * 0.34f, &m);
    int tx = (int)m.x;
    { static const char *SM[3] = { "OFF", "FLG", "PHS" };   // 3-state: OFF kills the sweep (regardless of the knob), FLG/PHS pick the flavour
      if (lcdbtnf(0x2D8u, &tx, (int)m.y, (int)m.h - 1, SM[mswmode], mswmode != 0)) mswmode = (mswmode + 1) % 3; }
    static const char *RB[3] = { "1BAR", "2BAR", "4BAR" };
