...
static void draw_808(void) {
    ...
    for (int s = 0; s < STEPS; s++) {
        ...
        if (c) {
            ...
            } else if (darmed == DD_PROB) {                  // FLAG/PROB — vertical slide on the pressed cell
                float f = clamp((94 - c->cy) / 13.0f, 0, 1);
                dprob[dsel][s] = snap_prob(f); dgrid[dsel][s] = 1;
            } else {                                         // FLAG/TUNE — bipolar p-lock offset (centre = follow the voice)
                dtoff[dsel][s] = clamp((89 - c->cy) / 8.0f, -1, 1); dgrid[dsel][s] = 1;
            }
        }
    ...
    for (int s = 0; s < STEPS; s++) {
        ...
        if (c) {
            ...
            if (dscreen != DS_FLAG) {                        // VCE/KIT — paint on/off
                ...
                d9toff[d9sel][s] = clamp((89 - c->cy) / 8.0f, -1, 1); d9grid[d9sel][s] = 1;
            }
        }
