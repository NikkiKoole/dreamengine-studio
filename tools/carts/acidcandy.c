...
static void draw_mst(void) {
    ...
    // ④ TEMPO knob in the LCD's right gutter (maps 0..1 → 60..200; label = the live BPM number).
    // This is what freed the bottom row so the LCD could grow.
    font(FONT_TINY); plabel("BPM", 147, 40, CLR_DARK_BROWN);
