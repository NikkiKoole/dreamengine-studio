...
#define VOW_REST (-1.0f)                     // a slot with no vowel: the voice pauses for that note
// OO AH EE (pause) AH EH OH (pause) — pedalboard's six vowels (its indices 0,2,4,2,3,1 over 4) split
// into two breath groups. EIGHT slots against a 16-step pattern that only advances on ON steps, so the
// phrase still lands differently bar to bar (the polymeter drift this cart already likes in its
// per-line loop lengths) — but now with an audible word boundary inside it.
static const float VOW_WORD[8] = { 0.0f, 0.5f, 1.0f, VOW_REST, 0.5f, 0.75f, 0.25f, VOW_REST };
#define VOW_NWORD 8
...
static void  vow_attack(void) {              // a fresh 303 note landed (NOT a tie — a tie holds, so it is no new syllable)
    ...
    float v = VOW_WORD[vowstep];
    if (v < 0.0f) vowgtgt = 1.0f - vowgap;   // a REST: hold the vowel where it is, close the voice by GAP
    else        { vowgtgt = 1.0f; vowtgt = v; }
}
    ...
    if (mstflow == 0) {
        ...
        if (fxpage < 0) {
            ...
            if (fxpage == FX_FORMANT) {
                ...
                // Knob COUNT follows the mode: 3 while hand-swept, 4 while speaking (the SWP page
                // already proved a 4-across row fits this glass). GAP only exists when SPK is lit, so
                // there is never a dead knob sitting there doing nothing.
                int nk = vowspeak ? 4 : 3;
                if (vowspeak) { lcdknob_cell(lay_grid(gc, nk, nk, 0, 2), &vowglide, "GLID", 0.35f);
                                lcdknob_cell(lay_grid(gc, nk, nk, 1, 2), &vowgap,   "GAP",  1.0f); }
...
static void r2_mstformant(Box b) {
    ...
    int nk = vowspeak ? 4 : 3;                                                           // GAP appears only while SPEAK is on
    if (vowspeak) { lcdknob_cell(lay_grid(m, nk, nk, 0, 3), &vowglide, "GLID", 0.35f);   // SPEAK owns the vowel → knob 1 becomes the slew
                    lcdknob_cell(lay_grid(m, nk, nk, 1, 3), &vowgap,   "GAP",  1.0f); }  // how deep a REST slot pauses
