...
static float msweep = 0.0f, msweepfb = 0.5f;                // master SWEEP: shared mix/depth macro (0 = off) + feedback (the "mental" intensity) for BOTH sweep devices
...
static void apply_fx(void) {
    ...
    static float aSwp = -2, aSwpFb = -2, aSwpRate = -2; static int aFlon = -1, aPhon = -1;
    {
        static const float SBARS[3] = { 1.0f, 2.0f, 4.0f };
        float srate = g_bpm / (240.0f * SBARS[mswdiv]);        // Hz: one LFO cycle per N bars (4 beats/bar)
        if (msweep != aSwp || msweepfb != aSwpFb || flon != aFlon || phon != aPhon || srate != aSwpRate) {
            float mix = msweep, dep = 0.55f + 0.45f * msweep, fb = msweepfb * 0.9f;
            if (flon && msweep >= 0.02f) flanger(srate, dep, fb, mix);       else flanger(0.3f, 0, 0, 0);       // FLANGER device
            if (phon && msweep >= 0.02f) phaser(srate, dep, fb, mix, 6);     else phaser(0.4f, 0, 0, 0, 6);     // PHASER device
            aSwp = msweep; aSwpFb = msweepfb; aFlon = flon; aPhon = phon; aSwpRate = srate;
        }
    }
...
static void draw_mst(Box stage) {
    ...
    if (mstflow == 0) {
        ...
        // SWEEP — TWO independent devices (FLG + PHS, run 2/1/0 at once) on top with the tempo-synced
        // rate; the shared SWEEP (mix/depth) + FB (feedback) knobs below. The engine's own LFO does the
        // swirl — arm a device and turn SWEEP up and the mix whooshes on its own.
...
// FLG + PHS + tempo-synced rate (1/2/4-BAR) flow on the LEFT; the SHARED SWEEP (mix/depth) + FB
// (feedback) knobs sit in the RIGHT third. The engine's own LFO does the swirl (see apply_fx).
static void r2_mstsweep(Box b) {
    Box m = lay_inset(b, 2);
    Box kn = lay_split(m, EDGE_RIGHT, m.w * 0.34f, &m);
    int tx = (int)m.x;
    if (lcdbtnf(0x2D8u, &tx, (int)m.y, (int)m.h - 1, "FLG", flon)) flon = !flon;   // FLANGER device on/off
    if (lcdbtnf(0x2DCu, &tx, (int)m.y, (int)m.h - 1, "PHS", phon)) phon = !phon;   // PHASER device on/off
    static const char *RB[3] = { "1BAR", "2BAR", "4BAR" };
    for (int r = 0; r < 3; r++) if (lcdbtnf(0x2D9u + r, &tx, (int)m.y, (int)m.h - 1, RB[r], mswdiv == r)) mswdiv = r;
    lcdknob_cell(lay_grid(kn, 2, 1, 0, 2), &msweep,   "SWEEP", 0.0f);
    lcdknob_cell(lay_grid(kn, 2, 1, 1, 2), &msweepfb, "FB",    0.5f);
}
...
static void r2_bigscreen(Box c, int focus) {
    ...
    // MST — mixer + PCF/CRU/GAT lanes all at once; GEN / SONG / DLY are chip-revealed panels (like 303/drums)
    {
        ...
        int panelH = (open == 2 || open == 4) ? 22 : 13;            // SONG (6 slots) + SWEEP (knobs) taller; GEN/DLY are one row
        int botH = chipH + (open ? panelH + 1 : 0);
