/* de:meta
{
  ...
  "todo": [
    ...
    "drum step PROBABILITY — the biggest modern-drum-machine feature acidrack lacks (Elektron → Behringer RD-8/RD-9 all added it): each active drum step gets a % chance to fire, so hats/fills breathe and the loop never repeats identically. THE house/techno move for a living pattern out of a static grid. Cheap: a per-cell 0-100 (or 4-5 buckets) rolled against a per-step RNG at trigger time. Highest bang-for-buck here. Pairs with per-track length below. NOT greenfield — copy from pocketbox.c's Elektron trigger conditions (CONDS[] table + cond_ok() gate, incl. a 50% + X:Y loop conditions); turing.c is the pure per-step flip-probability engine (flip_prob()).",
