...
#define NMK (NFX + 2)   // master knobs: fxk[NFX] + the live FLT (idx NFX, unsaved) + SWG (idx NFX+1, the master shuffle — swing now drives drums AND both 303s, so it lives on the global strip; also Z/X)
...
static void draw_909_fx(int y0) {
    ...
    // (SWING moved to the MASTER strip's SWG knob — it drives drums + both 303s now, so it's global; Z/X keys still work)
    font(FONT_SMALL);
...
static void draw_master_panel(int y0) {
    Pattern *P = &bank[editBank];
    rectfill(2, y0, W() - 4, panel_h() - 2, CLR_BLACK);
    // knob row: TIME/FB (the shared delay unit) · GLU · PCF/RES · DRV · PUMP · FLT · SWG
    int avail = W() - 84, cols = mst_cols(), kp = avail / cols;   // wraps to 2 rows when narrow
    font(FONT_TINY);
    for (int k = 0; k < NMK; k++) {
        int cx = 22 + (k % cols) * kp, cy = y0 + 12 + (k / cols) * 22;
        float *v; const char *nm; int col; bool save;
        if      (k < NFX)     { v = &fxk[k]; nm = FXNAME[k]; col = CLR_MEDIUM_GREY;  save = true;  }
        else if (k == NFX)    { v = &djf;    nm = "FLT";     col = CLR_LIGHT_YELLOW; save = false; }  // live DJ filter — unsaved
        else                  { v = &swingf; nm = "SWG";     col = CLR_LIGHT_GREY;   save = true;  }  // master shuffle (0..1 → swing 50..66)
        if (ui_knob(v, cx, cy, "")) {
            if (k == NFX + 1) swing = 50 + (int)(swingf * 16.0f);   // SWG → swing (both 303s + drums follow it)
            if (save) mark_dirty();
        }
        print_rot(nm, cx - 12, cy, 270.0f, col, 1);
    }
    font(FONT_NORMAL);
