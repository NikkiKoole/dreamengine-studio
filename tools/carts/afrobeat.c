/* de:meta
{
  ...
  "todo": [
    ...
    "EFFECTS WIRING (design/afrobeat-effects-wants.md says READY TO BUILD): all six effects this station's brief voted for have SHIPPED. wah() + instrument_wah for the Gentleman chop, leslie() + instrument_leslie for the organ that 'is not really there yet', tape() for the Afrika-70 console glue, and glue()/multiband()/sidechain() for the compressed electric bass and the pumping band glue (compression WAS the doc's missing roster entry - it now ships three ways). The cart wires only instrument_drive and glue (`node tools/wants-check.js afrobeat` has the live list). Nothing is blocked; this is wiring."
  ],
