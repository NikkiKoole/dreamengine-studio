...
// canonical SPRITE-blit stress test — every bunny is one spr() of a 16×16 sprite,
// pre-tinted into 5 colours baked as slots 0..4 (NOT pal()-recoloured: pal() is
...
// settings → rendering, flip between
// "hardware (GPU)" and "software (CPU canvas)", and watch the held-bunny count
// move: the GPU rasterises 16×16 textured quads in hardware; the software canvas
// blits them on the CPU. Same cart, two engines, one honest number each.
//
// Physics is a FIXED per-frame step (no dt()) — deliberately. A benchmark wants
// the slowdown to be VISIBLE: when blits overrun the frame, the whole swarm goes
// slow-mo instead of teleporting, so you SEE the cliff you're measuring.
//
...
#define START_N  1200             // a lively swarm already bouncing on load
#define GRAVITY  0.30f
#define BOUNCE   0.85f            // floor restitution
#define BUNNY_SZ 16

// tint = sprite slot 0..4 (pre-tinted in bunnymark.cart.js): white/yellow/pink/lime/orange
...
void init(void) {
    colorkey(0);                        // index 0 = transparent in the bunny sprite
    n = 0; best_held = 0;
...
void draw(void) {
    ...
    // bunnies — each is one plain spr() of its pre-tinted slot. No pal(): the
    // colour is baked into the sprite, so the GPU batches every bunny into one
    // draw call and the software canvas does a straight copy (no per-pixel
    // recolor). This is the whole benchmark — keep it a pure blit.
    for (int i = 0; i < n; i++)
        spr(bun[i].tint, (int)bun[i].x, (int)bun[i].y);
