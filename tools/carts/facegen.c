/* de:meta
{
  ...
  "todo": [
    ...
    "extract facegen.h — pull Npc + roll_npc() + draw_portrait()/draw_avatar() into a reusable header so the GTA-style game #includes it (HIGHEST VALUE — the whole point is to lift this into the game)."
  ]
}
de:meta */
#include "studio.h"
...
//   (Extract facegen.h is now tracked in de:meta.todo[] — node tools/cart-todos.js facegen.)
