/* de:meta
{
  ...
  "description": "A multitouch FRETLESS string instrument: touch a string and your finger's POSITION along it is the pitch, so a chord is your finger GEOMETRY, not a button you press. The payoff of the monochord experiment, and the honest version of pedalboard's chord buttons (here you actually form the shape). Six strings, standard tuning E A D G B e, drawn in TAB ORDER (high e on top, low E at the bottom). The faint ticks are semitone marks (the bright one is the 12th, an octave up) and nothing snaps to them, so intonation is on you: a straight vertical rake is the open tuning moved up the neck (a moveable barre), angled fingers voice everything else. TOUCH a string = pluck it there; SLIDE along it = glide the pitch, both ways; DRAG across the strings = a strum (each sounds as the finger crosses it). SPACE strums all six at the MOUSE's position, a barre you can slide, which is the whole desktop crutch since you can't multitouch a mouse. One ringing INSTR_PD voice per string (Casio phase-distortion, an oscillator, so a held note really does glide). A full six-finger chord plus a strum exceeds the iPhone 5-touch cap, so it leans desktop/iPad.",
  "todo": [
...
//
// Standard tuning E A D G B e. The voice is INSTR_PD (phase distortion), an oscillator, so a
// held note glides both ways: nothing here quantizes to a fret. Two-handed
...
// neck geometry — strings run left→right in TAB ORDER: low E (string 0) at the BOTTOM, high e on
// top (higher pitch, higher on the page — what every guitarist reads). Drawing index 0 on top
// mirrors the neck and reads as a LEFT-handed guitar facing you. Only the row mapping flips; the
// string arrays (OPEN/SNAME/vH/…) and the low→high strum order stay index-ordered.
#define NUT_X    34
...
static int   str_y(int s)   { return STR_TOP + (NSTR - 1 - s) * STR_GAP; }   // string index → screen row (inverted: tab order)
...
void draw(void) {
    ...
    int y0 = str_y(NSTR - 1), y1 = str_y(0);   // top edge = high e, bottom edge = low E
    ...
    // strings — low E (thick) at the BOTTOM, high e on top (tab order); glow as they ring, brightest on the pluck attack
