/* de:meta
{
  ...
  "todo": [
    ...
    "ENGINE PROMOTION - lift the cart-land UNISON construction into a real engine primitive (the '>=2 carts want it -> promote' rule; recipe->macro->engine, ADR-0016/0017). The 'mt70 MULTI-SLOT trick' (N detuned oscillator slots summed = supersaw) is genuinely hand-rolled in 3 analog-voice carts - mt70 (namesake, 1-3 stacked slots), moog (3 detuned oscs), filterenv (2 saws + sub) - plus a lighter single-slot 'unison detune' in groovebox; and motionbox now wants it for the lead. (Verified by reading each - an earlier list of ~8 was a loose grep; air/martenot/cr78/gamelan were false 'detune'/'fat' matches.) That's the promotion trigger: a first-class UNISON primitive (e.g. instrument_unison(slot, voices, detune) / INSTR_SUPER) so carts stop hand-managing a slot pool + firing across it. SHOWCASE CART when it lands: 'supersaw' - a Roland JP-8000 Super Saw trance lead (7 detuned saws, THE most-unison synth ever made); the detune-BLOOM gesture (one thin saw opening into a wall of 7) IS the primitive's reason to exist, and it's the EXTREME end that moog's 3-osc fat doesn't cover. NOTE: dreamengine-wide ENGINE work, not motionbox-only. (CHORD is NOT a candidate - chord(root, CHORD_*, ...) is already an engine primitive, ~40 carts use it. 'pd' is the Casio-CZ phase-distortion ENGINE, also already in-engine - unrelated to unison.)",
