/* de:meta
{
  ...
  "todo": [
    ...
    "UNISON / CHORD / SYNC for the lead - NOT engine work (an earlier todo wrongly said 'no primitive'; there is): chord-from-one-note is the built-in chord(root, CHORD_*, instr, vol) (9 types); a fat supersaw is the multi-slot detune trick - fire the same note on a small POOL of extra slots, each instrument_tune'd a few cents apart, summed (zero engine code); and instrument_sync(slot, ratio) is a one-call hard-sync fat lead. The real cost of the hand-roll is a CART refactor: a part owns ONE slot today, so a slot-pool lead needs firing the note across it. BUT if the engine-promotion below lands (instrument_unison), the lead stays ONE slot and this refactor evaporates - so prefer waiting for the primitive over hand-rolling here. Do after save/recall.",
    "ENGINE PROMOTION - lift the cart-land UNISON construction into a real engine primitive (the '>=2 carts want it -> promote' rule; recipe->macro->engine, ADR-0016/0017). The trick: N detuned copies of ONE waveform summed = supersaw. CORRECTED TRIGGER (2026-07-09, verified by READING each cart's voice-firing, not a grep): the earlier list ('3 analog-voice carts + groovebox') was overstated - moog and mt70 are NOT unison and will NOT migrate. moog stacks 3 INDEPENDENT oscillators (each its own wave/octave/level, OSC2 hard-sync, per-voice drift, live per-frame detune ride via note_pitch) - multi-VOICE architecture, not detuned copies. mt70 stacks sines at HARMONIC RATIOS (+19.02 semis = ratio 3.0) with per-partial vol/decay - additive synthesis, folding it into unison is a category error. The GENUINE unison demand is: groovebox's pad (single-slot instrument_tune shimmer), filterenv's saw PAIR (SLOT+SLOT2; its 3rd slot is an octave-down triangle SUB, not unison), motionbox's lead (this cart), and the showcase below. So the promotion stands on NEW demand, not on 4 carts collapsing in - still a valid ADR-0016/0017 promotion, just honestly grounded. NB instrument_tune(slot, semitones) already exists but is a single constant per-slot offset, NOT internal unison. API STILL TO SETTLE (design doc, in line with the other live tweakables): must ride detune LIVE + be an LFO/ENV/MOTION target (a LFO_DETUNE/ENV_DETUNE) so the detune BLOOM is motion-recordable (the whole point for this cart), and likely a stereo-WIDTH param (half the JP-8000 magic is the detuned voices panned across the field). SHOWCASE CART when it lands: 'supersaw' - a Roland JP-8000 Super Saw trance lead (7 detuned saws, THE most-unison synth ever made); the detune-BLOOM gesture (one thin saw opening into a wall of 7) IS the primitive's reason to exist, the EXTREME end that moog's 3-osc fat doesn't cover. NOTE: dreamengine-wide ENGINE work, not motionbox-only. (CHORD is NOT a candidate - chord(root, CHORD_*, ...) is already an engine primitive, ~40 carts use it. 'pd' is the Casio-CZ phase-distortion ENGINE, also in-engine - unrelated to unison.)",
