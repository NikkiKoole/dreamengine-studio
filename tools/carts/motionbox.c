/* de:meta
{
  ...
  "todo": [
    ...
    "ENGINE - true UNISON + CHORD oscillator models (the MMT heart we couldn't reach): the OSC page has WAVE but the engine has no supersaw (N detuned oscs stacked) or chord-from-one-note primitive. Adding INSTR_UNISON / a unison spread to sound.h would let the LEAD get its thick supersaw. Cross-cutting engine work, not a cart-only change - benefits every synth cart.",
