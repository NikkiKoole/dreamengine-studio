/* de:meta
{
  ...
  "todo": [
    "EFFECTS WIRING (design/napoleon-effects-wants.md says READY TO BUILD): every effect this station's brief asked for has SHIPPED - reverb, reverb_spring (the SERENADE plate), tape, chorus, wah, and the gate this doc flagged as a missing roster entry. The cart still calls ZERO bus effects. FOREVER's gated snare is a written-down recipe: FX_GATE after FX_REVERB in fx_order (see effects-recipes.md 'gated reverb'). Nothing is blocked; this is wiring."
  ],
