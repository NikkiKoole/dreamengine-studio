...
static int    n_obld;
static OsmBld cgbld[CGB_MAX];                 // Rung 7: citygen city buildings, world-px + home chunk
static int    n_cgbld;                        // (pre-converted from citygen's metre boxes on cg_toggle)
...
void update(void) {
    ...
    { RoadHit _rh = road_at(sx, sy);   // P1 seam — deterministic trace of road_at() AT THE RIG (drives handling)
      ...
      watch("rn2", "%d", rn2_on); watch("cg", "%d", cg_on); watch("cgbld", "%d", n_cgbld); }
...
static int gen_chunk(int cx, int cy, Obstacle *out) {
    int k = 0;
    if (rn2_on) return 0;                     // Rung A1: nothing solid over the spine yet (buildings = A2)
    if (cg_on) {                              // Rung 7: the generated city's terraces (citygen cg_lots)
        int roof[4] = { CLR_BROWN, CLR_RED, CLR_DARK_PURPLE, CLR_DARK_GREY };
        for (int b = 0; b < n_cgbld && k < OB_PERCHUNK; b++) {
            if (cgbld[b].cx != cx || cgbld[b].cy != cy) continue;
            Obstacle o; ob_init(&o, cx, cy, k);
            o.kind = OB_HOUSE;                                    // solid — holds till a heavy hit smashes it
            o.bx = o.x = cgbld[b].wx; o.by = o.y = cgbld[b].wy;
            o.hw = cgbld[b].hw; o.hh = cgbld[b].hh; o.ang = cgbld[b].ang;
            o.gw = 3; o.gh = 3;                                   // 3×3 brick → house-like mass/strength
...
static void cg_toggle(void) {
    ...
    if (cg_on) {
        if (citygen_pick_city(RN2_SPAWN_X, RN2_SPAWN_Y)) {
            ...
            if (citygen_nearest_street(ar_cx, ar_cy, &rx, &ry, &rang)) {   // (builds + caches the city)
                cg_ox = rx; cg_oy = ry; ang = rang;
            } else { cg_ox = ar_cx; cg_oy = ar_cy; }
            for (int b = 0; b < cgb_n && n_cgbld < CGB_MAX; b++) {   // metre boxes → world-px + home chunk
                float wx = (cgb_mx[b] - cg_ox) * CG_PPM, wy = (cgb_my[b] - cg_oy) * CG_PPM;
                cgbld[n_cgbld].wx = wx; cgbld[n_cgbld].wy = wy;
                cgbld[n_cgbld].hw = cgb_w[b] * 0.5f * CG_PPM; cgbld[n_cgbld].hh = cgb_h[b] * 0.5f * CG_PPM;
                cgbld[n_cgbld].ang = cgb_ang[b];
                cgbld[n_cgbld].cx = ifloordiv((int)wx, CHUNK); cgbld[n_cgbld].cy = ifloordiv((int)wy, CHUNK);
                n_cgbld++;
            }
