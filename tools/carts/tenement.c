/* de:meta
{
  ...
  "todo": [
    "WHERE THIS STANDS AFTER 2026-08-13, because the maker's verdict is now half-answered and the ORDER below changed. The verdict was that the cart reads as an architectural diagram with a caption. The rendering was replaced — low-poly triangles with a depth buffer, real palette ramps, free orbit and tilt, and three POSES (stand, sit, work) where before there was one and a half. That bought the two legibility items that were about IDENTITY: you can see whose flat someone is in, and you can see what they are doing. What it did NOT buy is the two that are about STAKES and STATE, and those are now the top of the list. The look is no longer the bottleneck; the sim having nothing to say is.",
    "POSTURE MUST CARRY THE WORST NEED — the remaining half of the legibility item, and now the most valuable thing on this list. Poses say what a resident is DOING (rest, use, labour) and nothing says how badly it is going: the HUD prints 'filthy' and 'bursting' in a text band while the figure stands there unbothered. Everything needed is already in place — three meshes, per-household colour, a depth buffer that does not care about overlap — so this is a shading and a slump away, not new machinery. Do it before any more sim work: it is the piece that makes the argmax's decisions visible as a MOOD rather than a number.",
    "LET SOMETHING BE LOST, and this is the real design conversation rather than a task. Nothing in this sim can go wrong: econ.h says so in its own header ('no eviction, no game over, no score, no consequence') and arrears is 'A RECORD, not a punishment' that nothing reads. Needs bottom out and the resident carries on. There is also no MEMORY: every minute is independent, so nothing accumulates into a situation you could name, no day is worse than yesterday, and no mess is anybody's fault. That was a principled deferral (design section 8a leaves the win condition open on purpose) but principled deferrals add up to a cart with no stakes, and no amount of argmax honesty substitutes. Eviction is the obvious version; a smaller one is needs that leave TRACES the building keeps, so a neglected flat LOOKS neglected and today's mess is yesterday's fault. Now unblocked: you can finally see who lives there.",
    ...
    "A RIM ON THE RESIDENTS is still open and still cheap, and it is the other half of counting people: two residents on neighbouring tiles read as one blob, so a queue of three looks like a queue of one. Per-household colour helped and did not solve it — two members of the SAME household adjacent are still one shape. With the depth buffer in place a rim is a per-pixel job rather than a sort problem.",
    "NO POSE FOR HAUL: a resident carrying an item looks identical to one walking empty-handed, and hauling is how goods move in this sim. The pattern to copy is the work pose — read the sim's own answer (`carrying` is a live item index, so it needs no new state) and give it a silhouette, most obviously something held in front. Same rule as the others: at this size the outline carries it, detail is noise.",
    "THE POLYGON VIEW IS NOT MEASURED ON A DEVICE. 2.66ms average against the sprite view's 0.49ms, about 15.5k one-row rectfill a frame — a sixth of the 60fps budget on desktop, but ADR-0024's phone factor puts it near the edge. Run ios/measure-device.sh before this ships to iOS. Note both cheap levers are already SPENT: there is no sort to speed up, and subdivision is already deleted (it existed only to make a sort sound, and removing it paid for the depth test almost exactly). What is left if it must come down is real work — fewer triangles per mesh, or a coarser canvas.",
    ...
    "The D/R fork, still PARKED and now for a better reason than before. D (price an action by its duration) and R (a day-shaped bid plus a floor under the argmax) default OFF, so the shipped sim is still the one 242 assertions describe. Together they take contention from 3.3% of frames to 16.2% and are the first time design section 1's claim actually happens. Turning them on rewrites case 1's converse, the cart's headline assertion. The reason to wait is no longer 'you cannot see the residents' — you can now — it is that contention with no CONSEQUENCE is still just traffic. Do it after something can be lost. See design section 12; a live A/B is what the keys are for.",
