/* de:meta
{
  ...
  "todo": [
    ...
    "MAKE THE RESIDENTS THE LOUDEST THING ON SCREEN. Right now in a game about people sharing a building, the people are the LEAST visible thing on it: 1-tile blobs in the same colour family as the furniture and the floor, no rim, no contrast, so they read as more furniture. Everything a viewer knows comes from the text band ('exhausted', 'in the hall', 'nowhere to wash'), so the picture is an architectural diagram with a caption. Wants: a rim so they pop off the furniture (this also fixes the two-waiters-read-as-one-blob item below, so a queue of three can be COUNTED), per-household colour so you can see whose flat someone is standing in without reading the HUD, and posture carrying the worst need so exhausted/desperate/content are readable at a glance. Cheap, and it is the difference between 'some furniture' and 'four people'.",
