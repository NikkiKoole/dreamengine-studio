/* de:meta
{
  ...
  "todo": [
    ...
    "THE POLYGON VIEW COSTS ~5x THE SPRITE VIEW AND IS NOT YET MEASURED ON A DEVICE: 2.35ms average against 0.49ms, ~15.5k one-row rectfill a frame. Fine on desktop, a seventh of the 60fps budget, but ADR-0024's phone factor puts it near the edge — measure with ios/measure-device.sh before this ships to iOS. Note the two cheap levers are already SPENT: the path uses a depth buffer so there is no sort to speed up, and subdivision is already deleted (it existed only to make a sort sound, and removing it paid for the depth test exactly). What is left if it must come down is real work: fewer triangles per mesh, or a coarser canvas.",
    ...
    "THE SITTING POSE IS WIRED (2026-08-13) AND IT WAS A RENDERER GAP, NOT A SIM ONE — worth remembering as a class. TN_POSE_SIT has been in the contract since day one and offer.h has always said a sofa and a toilet are things you SIT on; art.h mapped everything except LIE to the standing figure, so for the cart's whole life a resident using the sofa was drawn upright ON TOP OF THE BACKREST. Two fixes: a person_sit model (both views, since the atlas is the one source of truth) and ISO_REST_Z, because the old code lifted a posed body to the object's full HEIGHT — right for a bed, where the mattress IS the top, wrong for anything with a backrest or a cistern above the seat. WORK LANDED TOO (same day): a resident on a 480-minute shift now stands with both arms out in front of it, and design section 4's promise that labour is VISIBLE is finally a thing you can see. It is selected from the winning bid's TAG rather than from the activity, because nothing in this cart ever sets TN_ACT_WORK so `activity` cannot tell a shift at the loom from a snack at the fridge. STILL MISSING: HAUL (a resident carrying an item looks identical to one walking empty-handed), and nothing at all distinguishes a resident who is DESPERATE from one who is content — the posture-carries-the-worst-need half of the item above.",
    "THE MESHES NOW EXIST TWICE — in runtime/tenement/art.h and in tools/carts/polyroom.c — and that is a real drift hazard rather than a tidiness complaint: polyroom's spec() is what GATES them (footprint parity, no interpenetration, and the scale identity against the sprite projection), so a mesh edited only in art.h is a mesh with no gate. Extracting them wants a shared header, and the honest blocker is ADR-0006: a library header wants real consumers, and two IS the threshold, so this is now due.",
