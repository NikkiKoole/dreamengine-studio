...
void update(void) {
    ...
    // C = the cymbal's band structure (tr808.h). 3BAND is Reid's schematic and the DEFAULT since the
    // owner's ear picked it (2026-07-28): the mid and high bands decay faster than the low one, so the
    // crash DARKENS as it rings instead of holding one colour. 1BAND is the single-band voice this cart
