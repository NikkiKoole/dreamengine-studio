/* de:meta
{
  ...
  "lineage": "The acceptance probe + showcase for sample_shift() — Rung B of docs/design/contemporary-rebirth.md, the INTERVAL face of the formant-preserving PSOLA that docs/design/transparent-autotune.md shipped as a scale-SNAP (sample_autotune). It answers the question that doc's gate exists for: did the pitch move, and did the LENGTH survive? Four takes of ONE captured voice, side by side: RAW, SNAPPED, an octave UP, an octave DOWN. Source is INSTR_VOICE captured through record_arm/record_grab, so it needs no mic and replays deterministically (unlike mictune/livetune, which are LIVE by nature).",
  "description": {
    "summary": "The chipmunk, on a dial. One captured voice, four ways: raw, auto-tuned to a scale, an octave up, an octave down — and all four are exactly the same LENGTH, which is the whole trick. Playing a sample higher normally makes it shorter; this doesn't.",
    "detail": "A synthesised 'ah' (INSTR_VOICE with a deliberately wobbly pitch) is captured off the console's own output with record_arm/record_grab, then copied into four sample slots and processed: slot 0 stays RAW; slot 1 gets sample_autotune (snapped to a scale, wobble ironed out); slot 2 gets sample_shift(+12) — an octave up; slot 3 gets sample_shift(-12) — an octave down. Formants ride along with the pitch, so the shifted takes sound pitched rather than re-voiced: this is the chipmunk, done without changing the duration. It auto-plays the four in order, drawing each waveform with the region being heard highlighted, so the numbers from tools/formant-check.js line up with what you hear: f0 doubles in slot 2 (440Hz) and halves in slot 3 (110Hz), while every waveform stays the same width on screen.",
  ...
  "todo": [
    ...
    "The SNAPPED take reads f0 179Hz with a 110-220Hz range in formant-check - the analyzer octave-flips on PSOLA output, not necessarily a fault in the take. Worth a look when the transparent-shift spike happens.",
    "POPPING: the DOWN-octave take is FIXED (2026-07-26, after the maker reported it by ear), the other two are NOT. Two engine bugs found and fixed, both guarded to the shift face so sample_autotune stayed byte-identical: zero grain overlap going down (177 hard splices -> 1) and a truncated fractional grain read (a zero-order-hold staircase). SNAPPED and UP+12 still pop faintly. That third defect is a phase problem in how source epochs map to output epochs, and TWO textbook fixes were tried and REVERTED because both regressed the snap face into period doubling (f0 220.4 -> 178.8 and -> 192.7, flipping 110-220): a WSOLA correlation phase-lock on the epoch marking, and a monotone accumulator replacing nearest-epoch selection. Both are now DO-NOT-DO comments in sound.h with the numbers attached. Do not re-try either without a harness.",
    "BUILD THE HARNESS FIRST (this is why the two attempts above failed). Three detectors are needed and each one was blind to a bug the others caught: a first-difference splice finder (scored snapped/up at ZERO - certified the takes the maker could hear as clean), a periodicity-break finder with the RAW take as control (ranked UP+12 worst of four, but blind to the staircase), and the f0 reading (the ONLY one that catches period doubling, because a period-doubled signal is still perfectly periodic and the periodicity metric scores it as a 2x IMPROVEMENT while it sounds worse). Promote all three into a committed psola-check.js beside formant-check.js, then re-attempt the phase fix.",
    "This cart is the acceptance probe for artifact-freeness, not just for pitch/length. Its four takes plus the RAW control are exactly the A/B a PSOLA change needs, so run it before and after any at_psola_slot edit."
  ]
}
de:meta */
//
