/* de:meta
{
  ...
  "todo": [
    ...
    "POPPING, mostly fixed 2026-07-26 after the maker reported it by ear (three engine bugs, all measured: zero grain overlap on down-shift, peak-picked epochs instead of phase-locked, and a truncated fractional grain read). DOWN-12 now measures cleaner than the raw source take. A faint residual remains on SNAPPED (1.13x the raw take's periodicity error) and UP+12 (1.22x, and still ~5x its glitch RATE): inherent PSOLA pulse duplication plus un-filtered decimation at fstep 2. Fixing that means an anti-aliased grain read and a smarter duplicate-pulse rule - the same spike as the transparent shift. Detectors used are worth rebuilding if you take this: a first-difference splice finder and a periodicity-break finder, because each one MISSED a bug the other caught.",
    "This cart is now the acceptance probe for artifact-freeness, not just for pitch/length. Its four takes plus the RAW control are exactly the A/B a PSOLA change needs, so run it before and after any at_psola_slot edit. Worth promoting the two detectors into a committed tool (a psola-check.js beside formant-check.js) so the numbers in this todo are reproducible rather than sitting in a scratchpad."
  ]
}
de:meta */
//
