/* DE_NO_RAYLIB scripted WAV (+ optional PPM) host for ladderface.
 *
 * Linux proof path when there is no Raylib / no xxd / no window
 * (play.js and build-nr.sh both need those). Twin of tools/clips/cloudhold/render-nr.c.
 *
...
static int write_ppm(const char *path, const uint32_t *pix, int w, int h) {
    ...
    for (int y = h - 1; y >= 0; y--) {
        for (int x = 0; x < w; x++) {
            ...
            // sw_pack is 0xAABBGGRR (R in the low byte) — not 0xAARRGGBB
            fputc( p        & 255, f);
            fputc((p >>  8) & 255, f);
            fputc((p >> 16) & 255, f);
        }
...
int main(int argc, char **argv) {
    ...
    const char *wav = (argc > 3) ? argv[3] : "/tmp/ladderface.wav";
