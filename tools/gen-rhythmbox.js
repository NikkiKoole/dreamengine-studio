...
// The in-game font is ASCII, so an em dash renders as '?' on the console. Names are
// therefore emitted ASCII-only, and the verbose databook heading is kept separately as
// `source` so a cart can display a short name without losing the provenance string.
const ascii = s => s.replace(/[\u2014\u2013]/g, '-').replace(/[^\x20-\x7e]/g, '?');
const shortName = s => {
  const m = /^TABLE \d \((M25\d A[A-Z])\)\s*[-\u2014]\s*RHYTHM (\d+)(?:\s*\(([^)]+)\))?/.exec(s);
  return m ? `${m[1]} ${m[2]}${m[3] ? ' ' + m[3] : ''}` : ascii(s);
};
const L = [];
...
L.push('// edge never happens. Stated in Elektor April 1976 p420, and confirmed by the silicon: the');
L.push('// M258/M259 datasheet traces "OUT 1 WITHOUT RETURN TO 1" against "OUT 5 WITH RETURN TO 1"');
L.push('// and lists per-output choice of the two as a mask option. 36 runs of adjacent marks exist in the SGS');
...
L.push('');
L.push('typedef struct {');
L.push('    const char        *name;     // short, ASCII, for display');
L.push('    const char        *source;   // the document heading it was read from, verbatim');
