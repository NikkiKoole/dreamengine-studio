...
//
//   node tools/tune-check.js                 render the sweep + full report
...
function run(opts) {
  ...
  if (!notes.length) throw new Error('no gated notes found in trace — did tunecheck.c build?')
  ...
  for (const nt of notes) {
    ...
    const s0 = Math.floor(a + span * 0.12)
    const s1 = Math.floor(a + span * 0.88)
    const expected = midiToFreq(nt.midi)
    const m = measurePitch(s, s0, s1, sr, expected)
    const row = {
    ...
    if (m) {
      ...
      row.measuredHz = +m.hz.toFixed(2); row.cents = +f.cents.toFixed(1)
  ...
  // grouped, per-engine
  let lastEng = null
  console.log(`tuning sweep — ${results.length} notes @ ${sr}Hz   (warn >${WARN_CENTS}¢, bad >${BAD_CENTS}¢)\n`)
  for (const r of results) {
    if (r.engine !== lastEng) { console.log(`${r.engineName}  (id ${r.engine})`); lastEng = r.engine }
    const m = mark(r.verdict)
    const cents = `${r.cents >= 0 ? '+' : ''}${r.cents.toFixed(1).padStart(5)}¢`
  ...
  // worst offenders (by absolute cents within the octave)
  const bad = results.filter(r => r.verdict === 'off' || r.verdict === 'OUT OF TUNE')
    .sort((a, b) => Math.abs(b.cents) - Math.abs(a.cents))
  // engines that play in a different octave than asked (musical info, not a tuning fault)
